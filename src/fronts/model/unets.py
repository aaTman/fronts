...
def unet(
    ...
    tensors["En1"] = unet_utils.convolution_module(
    ...
    upsample_tensor = unet_utils.upsample(
        tensors[f"En{levels}"],
    ...
    tensor_De1 = unet_utils.convolution_module(
        tensor_De1, filters=filter_num[0], name="De1", **module_kwargs
    )  # Convolution module
    tensors["output"] = unet_utils.deep_supervision_side_output(
        tensor_De1,
    ...
    for decoder in np.arange(1, levels - 1)[::-1]:
        ...
        for node in range(1, num_middle_nodes + 1):
            if node == 1:  # if on the first middle node at the given level
                upsample_tensor_for_middle_node = unet_utils.upsample(
            ...
            tensors[f"Me{decoder}-{node}"] = unet_utils.convolution_module(
            ...
            if decoder == 1:
                tensors[f"sup{decoder}-{node}"] = (
                    unet_utils.deep_supervision_side_output(
                        tensors[f"Me{decoder}-{node}"],
        ...
        tensors[f"De{decoder}"] = unet_utils.convolution_module(
...
def unet_plus(
    ...
    for encoder in np.arange(
        ...
        tensors[f"En{encoder}"] = unet_utils.convolution_module(
            pool_tensor,
            filters=filter_num[encoder - 1],
            name=f"En{encoder}",
            **module_kwargs,
    ...
    tensors[f"De{levels - 1}"] = unet_utils.convolution_module(
...
def unet_3plus(
    ...
    if first_encoder_connections is True:
        for full_connection in range(2, levels):
            tensors[f"1---{full_connection}_full-scale"] = (
                unet_utils.full_scale_skip_connection(
                    tensors[f"En1"],
                    level1=1,
    ...
    for encoder in np.arange(
        ...
        tensors[f"{encoder}---{encoder}_skip"] = (
            unet_utils.conventional_skip_connection(
    ...
    # Bottom encoder node
    tensors[f"En{levels}"] = unet_utils.max_pool(
        tensors[f"En{levels - 1}"], name=f"En{levels - 1}-En{levels}", **pool_kwargs
    )
    tensors[f"En{levels}"] = unet_utils.convolution_module(
        tensors[f"En{levels}"],
    ...
    if deep_supervision:
        tensors[f"sup{levels}_output"] = unet_utils.deep_supervision_side_output(
            tensors[f"En{levels}"],
            num_classes=num_classes,
            output_level=levels,
            name=f"sup{levels}",
    ...
    for feature_map in range(1, levels - 1):
        tensors[f"{levels}---{feature_map}_feature"] = (
            unet_utils.aggregated_feature_map(
                tensors[f"En{levels}"],
                level1=levels,
                level2=feature_map,
    ...
    for decoder in np.arange(1, levels)[::-1]:
        ...
        if decoder == levels - 1:
            tensors[f"De{decoder}"] = unet_utils.upsample(
                tensors[f"En{levels}"],
        ...
        if (
            ...
            tensors[f"sup{decoder}_output"] = unet_utils.deep_supervision_side_output(
                tensors[f"De{decoder}"],
                num_classes=num_classes,
...
def attention_unet(
    ...
    tensors[f"AG{levels - 1}"] = unet_utils.attention_gate(
    ...
    tensors[f"AG{levels - 2}"] = unet_utils.attention_gate(
    ...
    for decoder in np.arange(2, levels - 1)[::-1]:
        ...
        tensors[f"AG{decoder - 1}"] = unet_utils.attention_gate(
            tensors[f"En{decoder - 1}"],
            tensors[f"De{decoder}"],
            kernel_size,
            pool_size,
