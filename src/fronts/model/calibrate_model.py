...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        help="Dataset for which calibration will be performed. Options are 'training', 'validation', 'test'.",
    )
    parser.add_argument(
        "--domain", type=str, help="Domain for which calibration will be performed."
    )
    parser.add_argument(
        "--model_dir", type=str, help="Parent directory for the model(s)."
    )
    parser.add_argument("--model_number", type=int, help="Model number.")
    parser.add_argument(
        "--data_source", type=str, default="era5", help="Data source for the variables."
    )
    ...
    try:
        _ = model_properties[
            "calibration_models"
        ]  # Check to see if the model has already been calibrated before
    except KeyError:
    ...
    for front_label in front_types:
        ...
        for boundary, color in enumerate(boundary_colors):
            ...
            x = [
                threshold
                for threshold, frequency in zip(
                    thresholds[1:], observed_relative_frequency[boundary]
                )
            ...
            axs[0].plot(
                ...
                color=color,
                linewidth=1,
                label="%d km" % ((boundary + 1) * 50),
            )
            axs[1].plot(
                x,
                calibrated_probabilities[boundary],
                color=color,
                linestyle="--",
                linewidth=1,
                label=r"%d km ($R^2$ = %.3f)" % ((boundary + 1) * 50, r_squared),
            )
            model_properties["calibration_models"][args["domain"]][front_label][
                "%d km" % ((boundary + 1) * 50)
            ] = ir
        ...
        plt.suptitle(
            f"Model {args['model_number']} reliability/calibration: {FRONT_NAMES[front_label]}"
        )
        plt.savefig(
            f"%s/model_%d/model_%d_calibration_%s_%s.png"
            % (
                args["model_dir"],
                args["model_number"],
                args["model_number"],
