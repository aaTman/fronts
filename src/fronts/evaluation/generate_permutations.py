...
def shuffle_inputs(image, labels):
    ...
    for var_num in var_nums:
        for lvl_num in lvl_nums:
            ...
            lon_indices = tf.random.uniform(
                [num_elements], 0, image.shape[0] - 1, dtype=tf.int32
            )
            lat_indices = tf.random.uniform(
                [num_elements], 0, image.shape[1] - 1, dtype=tf.int32
            )

            if lvl_num is None:
                pressure_level_indices = tf.random.uniform(
                    [num_elements], 0, image.shape[2] - 1, dtype=tf.int32
                )
            else:
                pressure_level_indices = tf.cast(
                    tf.fill([num_elements], lvl_num), tf.int32
                )

            if var_num is None:
                variable_indices = tf.random.uniform(
                    [num_elements], 0, image.shape[3] - 1, dtype=tf.int32
                )
            else:
                variable_indices = tf.cast(tf.fill([num_elements], var_num), tf.int32)

            indices = tf.stack(
                [lon_indices, lat_indices, pressure_level_indices, variable_indices],
                axis=-1,
            )
            image = tf.tensor_scatter_nd_update(
                image, indices, tf.reshape(values_to_shuffle, [num_elements])
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Input directory for the tensorflow dataset(s).",
    )
    parser.add_argument(
        "--model_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Number that the model will be assigned.",
    )
    parser.add_argument(
        "--baseline",
        action="store_true",
        help="Calculate baseline loss values for the permutations.",
    )
    parser.add_argument(
        "--single_pass", action="store_true", help="Perform single-pass permutations."
    )
    parser.add_argument(
        "--multi_pass", action="store_true", help="Perform multi-pass permutations."
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    try:
        file_loader = fm.DataFileLoader(
            args["tf_indir"], data_file_type="era5-tensorflow"
        )
        file_loader.test_years = test_years
        file_loader.pair_with_fronts(
            args["tf_indir"], underscore_skips=len(front_types)
        )
    except IndexError:
        file_loader = fm.DataFileLoader(
            args["tf_indir"], data_file_type="era5-tensorflow"
    ...
    try:
        optimizer_args = {
            arg: model_properties[arg] for arg in ["learning_rate", "beta_1", "beta_2"]
        }
        optimizer = getattr(tf.keras.optimizers, model_properties["optimizer"])(
            **optimizer_args
        )
    except KeyError:
        optimizer = getattr(tf.keras.optimizers, model_properties["optimizer"][0])(
            **model_properties["optimizer"][1]
        )
    ...
    permutations_file = "%s/model_%d/permutations_%d_%s.pkl" % (
        args["model_dir"],
        args["model_number"],
        args["model_number"],
        domain,
    ...
    permutations_dict = (
        ...
        else pd.read_pickle(permutations_file)
    )
    ...
    if "baseline" not in permutations_dict or args["baseline"]:
        ...
        baseline = np.array(
            [
                prediction[1 + num_outputs + front_no]
                for front_no in range(num_classes - 1)
            ]
        )
    ...
    if "single_pass" not in permutations_dict or args["single_pass"]:
        assert "baseline" in permutations_dict, (
            "Must calculate baseline values prior to permutations!"
        )
        baseline = permutations_dict["baseline"]
        permutations_dict["single_pass"] = (
            dict()
            if "single_pass" not in permutations_dict
            else permutations_dict["single_pass"]
        )
        ...
        combinations.extend(
            list(combo) for combo in itertools.product(variables, pressure_levels)
        )
    ...
    if args["multi_pass"]:
        single_pass_results = np.array(
            [permutations_dict["single_pass"][var] for var in variables]
        )
        ...
        baseline = permutations_dict["baseline"]
        permutations_dict["multi_pass"] = (
            dict()
            if "multi_pass" not in permutations_dict
            else permutations_dict["multi_pass"]
        )
        ...
        level_nums = None
        for front_no, front_type in enumerate(front_types):
            permutations_dict["multi_pass"][front_type] = (
                dict()
                if front_type not in permutations_dict["multi_pass"]
                else permutations_dict["multi_pass"][front_type]
            )

            permutations_dict["multi_pass"][front_type][
                variables[most_important_variable[front_no]]
            ] = np.max(single_pass_results, axis=0)[front_no]
            ...
            variable_nums = np.array(
                [most_important_variable[front_no], 0], dtype=np.int32
            )  # the 0 is a placeholder that will be overwritten iteratively

            variable_shuffle_order = np.arange(
                0, len(variables)
            )  # variables to shuffle
            variable_shuffle_order = np.delete(
                variable_shuffle_order, shuffled_parameters
            )  # remove pre-shuffled indices

            while len(variable_shuffle_order) > 0:
                print(
                    f"---> {front_type}: shuffling %s"
                    % ", ".join(variables[param] for param in shuffled_parameters)
                )
                temp_importance_list = np.array(
                    []
                )  # list that will be used to temporarily store importance values for the current round
                ...
                for var_to_shuffle in variable_shuffle_order:
                    ...
                    loss_by_type = np.array(
                        [
                            losses[1 + num_outputs + front_no]
                            for front_no in range(num_classes - 1)
                        ]
                    )
                    importance = np.round(
                        100
                        * (loss_by_type[front_no] - baseline[front_no])
                        / baseline[front_no],
                        3,
                    )
                ...
                most_important_variable_for_round = variable_shuffle_order[
                    np.argmax(temp_importance_list)
                ]
                permutations_dict["multi_pass"][front_type][
                    variables[most_important_variable_for_round]
                ] = np.max(temp_importance_list)

                shuffled_parameters = np.append(
                    shuffled_parameters, most_important_variable_for_round
                )
                variable_nums[-1] = (
                    most_important_variable_for_round  # add the round's most important variable to the list of variables to shuffle
                )
                variable_shuffle_order = np.delete(
                    variable_shuffle_order, np.argmax(temp_importance_list)
                )  # remove the round's most important variable for the next round
                variable_nums = np.append(
                    variable_nums, 0
                )  # add another index for variables in the next round
        ...
        variable_nums = None
        single_pass_results = np.array(
            [permutations_dict["single_pass"][lvl] for lvl in pressure_levels]
        )
        ...
        for front_no, front_type in enumerate(front_types):
            ...
            permutations_dict["multi_pass"][front_type][
                pressure_levels[most_important_level[front_no]]
            ] = np.max(single_pass_results, axis=0)[front_no]
            ...
            level_nums = np.array(
                [most_important_level[front_no], 0], dtype=np.int32
            )  # the 0 is a placeholder that will be overwritten iteratively

            level_shuffle_order = np.arange(
                0, len(pressure_levels)
            )  # levels to shuffle
            level_shuffle_order = np.delete(
                level_shuffle_order, shuffled_levels
            )  # remove pre-shuffled indices

            while len(level_shuffle_order) > 0:
                print(
                    f"---> {front_type}: shuffling %s"
                    % ", ".join(pressure_levels[param] for param in shuffled_levels)
                )
                temp_importance_list = np.array(
                    []
                )  # list that will be used to temporarily store importance values for the current round
                ...
                most_important_level_for_round = level_shuffle_order[
                    np.argmax(temp_importance_list)
                ]
                permutations_dict["multi_pass"][front_type][
                    pressure_levels[most_important_level_for_round]
                ] = np.max(temp_importance_list)

                shuffled_levels = np.append(
                    shuffled_levels, most_important_level_for_round
                )
                level_nums[-1] = (
                    most_important_level_for_round  # add the round's most important level to the list of levels to shuffle
                )
                level_shuffle_order = np.delete(
                    level_shuffle_order, np.argmax(temp_importance_list)
                )  # remove the round's most important level for the next round
                level_nums = np.append(
                    level_nums, 0
                )  # add another index for levels in the next round
