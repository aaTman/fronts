...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--forecast_hour", type=int, help="Forecast hour for the GDAS data"
    )
    parser.add_argument(
        "--front_dilation",
        type=int,
    ...
    parser.add_argument(
        "--fronts_netcdf_indir",
        type=str,
        help="Main directory for the netcdf files containing frontal objects.",
    )
    parser.add_argument(
        "--data_source",
        type=str,
    ...
    parser.add_argument(
        "--deterministic", action="store_true", help="Plot deterministic splines."
    )
    parser.add_argument(
        "--targets", action="store_true", help="Plot ground truth targets/labels."
    )
    parser.add_argument(
        "--open_contours", action="store_true", help="Plot probability contours."
    )
    parser.add_argument(
        "--filled_contours", action="store_true", help="Plot probability contours."
    )
    ...
    if args["deterministic"] and args["targets"]:
        raise TypeError(
            "Cannot plot deterministic splines and ground truth targets at the same time. Only one of --deterministic, --targets may be passed"
        )

    DEFAULT_COLORBAR_POSITION = {
        "conus": 0.75,
        "full": 0.85,
        "goes-merged": 0.88,
        "hrrr": 0.77,
    ...
    timestep = np.datetime64("%d-%02d-%02dT%02d" % (year, month, day, hour)).astype(
        object
    )
    ...
    try:
        probs_file = (
            f"{args['model_dir']}/model_{args['model_number']}/probabilities/model_{args['model_number']}_pred_{args['domain']}_{year}%02d.nc"
            % month
        )

        plot_filename = "%s/model_%d/maps/model_%d_%d%02d%02d%02d_%s.png" % (
            args["model_dir"],
            args["model_number"],
            args["model_number"],
            year,
        ...
        probs_ds = xr.open_mfdataset(probs_file).sel(
            time=[
                "%d-%02d-%02dT%02d" % (year, month, day, hour),
            ]
        )

        if args["forecast_hour"] is not None:
            fronts_file = "%s/%d%02d/FrontObjects_%d%02d%02d%02d_f%03d_%s.nc" % (
                args["fronts_netcdf_indir"],
                year,
                month,
                year,
                month,
            ...
            fronts_file = "%s/%d%02d/FrontObjects_%d%02d%02d%02d_%s.nc" % (
                args["fronts_netcdf_indir"],
                year,
                month,
                year,
        ...
        if args["forecast_hour"] is not None:
            forecast_timestep = (
                ...
                + np.timedelta64(args["forecast_hour"], "h").astype(object)
            )
            new_year, new_month, new_day, new_hour = (
                forecast_timestep.year,
                forecast_timestep.month,
                forecast_timestep.day,
                forecast_timestep.hour - (forecast_timestep.hour % 3),
            )
            fronts_file = "%s/%s%s/FrontObjects_%s%s%s%02d_f%03d_full.nc" % (
                args["fronts_netcdf_indir"],
                new_year,
                new_month,
                new_year,
            ...
            filename_base = f"model_%d_{year}%02d%02d%02d_%s_%s_f%03d" % (
                args["model_number"],
                month,
                day,
                hour,
            ...
            fronts_file = "%s/%d%02d/FrontObjects_%d%02d%02d%02d_full.nc" % (
                args["fronts_netcdf_indir"],
                year,
                month,
                year,
            ...
            filename_base = f"model_%d_{year}%02d%02d%02d_%s" % (
                args["model_number"],
                month,
                day,
                hour,
        ...
        plot_filename = "%s/model_%d/maps/%s-same.png" % (
            args["model_dir"],
            args["model_number"],
            filename_base,
        )
    ...
    if args["targets"]:
        ...
        try:
            if args["data_source"] in ["era5", "gfs", "gdas"]:
                fronts = xr.open_dataset(fronts_file).sel(
                    longitude=slice(extent[0], extent[1]),
                    latitude=slice(extent[3], extent[2]),
                )
            elif args["data_source"] == "hrrr":
                fronts = xr.open_dataset(fronts_file).isel(
                    y=slice(0, 1056), x=slice(0, 1728)
                )
                hrrr_coords = xr.open_dataset(
                    "%s/coordinates/hrrr.nc" % os.getcwd()
                ).isel(y=slice(0, 1056), x=slice(0, 1728))
                fronts = fronts.assign_coords(
                    {
                        "latitude": (("y", "x"), hrrr_coords["latitude"].to_numpy()),
    ...
    mask, prob_int = (
        args["prob_mask"][0],
        args["prob_mask"][1],
    )  # Probability mask, contour interval for probabilities
    vmax, cbar_tick_adjust, cbar_label_adjust, n_colors = (
        1.01,
        prob_int,
        10,
        (1 / prob_int) + 1,
    ...
    cmap_front = colors.ListedColormap(
    ...
    probs_ds = (
        probs_ds.isel(time=0)
        if args["data_source"] == "era5"
        else probs_ds.isel(time=0, forecast_hour=0)
    )
    probs_ds = probs_ds.transpose(*transpose_dims)

    for key in list(probs_ds.keys()):
        if args["deterministic"]:
            spline_threshold = model_properties["front_obj_thresholds"][args["domain"]][
                key
            ]["100"]
            probs_ds[f"{key}_obj"] = (
                transpose_dims,
                skeletonize(
                    xr.where(probs_ds[key] > spline_threshold, 1, 0).values.copy(
                        order="C"
                    )
                ),
            )
        ...
        if args["calibration"] is not None:
            try:
                ir_model = model_properties["calibration_models"][args["domain"]][key][
                    "%d km" % args["calibration"]
                ]
            except KeyError:
                ir_model = model_properties["calibration_models"]["conus"][key][
                    "%d km" % args["calibration"]
                ]
            original_shape = np.shape(probs_ds[key].values)
            probs_ds[key].values = ir_model.predict(
                probs_ds[key].values.flatten()
    ...
    if not args["open_contours"]:
        if len(front_types) > 1:
            ...
            for combination in all_possible_front_combinations:
                probs_ds[combination[0]].values = np.where(
                    probs_ds[combination[0]].values
                    > probs_ds[combination[1]].values - 0.02,
                    probs_ds[combination[0]].values,
                    0,
                )
    ...
    if args["data_source"] != "era5":
        valid_time = timestep + np.timedelta64(args["forecast_hour"], "h").astype(
            object
        )
        data_title = (
            f"Run: {args['data_source'].upper()} {year}-%02d-%02d-%02dz F%03d \nPredictions valid: %d-%02d-%02d-%02dz"
            % (
                month,
                day,
                hour,
        ...
        data_title = (
            "Data: ERA5 reanalysis %d-%02d-%02d-%02dz\n"
            "Predictions valid: %d-%02d-%02d-%02dz"
    ...
    for front_no, front_key, front_name, front_label, cmap in zip(
        range(1, len(front_names_by_type) + 1),
        front_types,
        front_names_by_type,
        front_types,
        contour_maps_by_type,
    ):
        if args["filled_contours"]:
            cmap_probs, norm_probs = (
                cm.get_cmap(cmap, n_colors),
                colors.Normalize(vmin=0, vmax=vmax),
            )
            probs_ds[front_key].plot.contourf(
                ax=ax,
                x="longitude",
                y="latitude",
                norm=norm_probs,
            ...
            cbar_ax = fig.add_axes(
                [cbar_position + (front_no * 0.015), 0.24, 0.015, 0.64]
            )
            cbar = plt.colorbar(
        ...
        if args["open_contours"]:
            probs_ds[front_key].plot.contour(
                ax=ax,
                x="longitude",
                y="latitude",
                levels=levels,
                colors=front_colors_by_type[front_no - 1],
                transform=ccrs.PlateCarree(),
                alpha=0.75,
                add_colorbar=False,
            )
        ...
        if args["deterministic"]:
            ...
            cmap_deterministic = colors.ListedColormap(
                ["None", front_colors_by_type[front_no - 1]], name="from_list", N=2
            )
            norm_deterministic = colors.Normalize(vmin=0, vmax=1)
            probs_ds[f"{front_key}_obj"].plot(
                ax=ax,
                x="longitude",
                y="latitude",
                cmap=cmap_deterministic,
