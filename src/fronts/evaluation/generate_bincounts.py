...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Base directory for the input netcdf files.",
    )
    parser.add_argument(
        "--bincount_outdir",
        type=str,
        required=True,
        help="Output directory for the netcdf files containing bincounts.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="era5",
        help='Data source (e.g., "era5", "MERGIR", etc.)',
    )
    ...
    netcdf_files = list(
        sorted(
            glob(
                args["netcdf_indir"]
                + "/%d%02d/%s*_%d%02d*.nc"
                % (year, month, args["data_source"], year, month)
            )
        )
    )
    ...
    if args["data_source"] == "era5":
            ...
            for var_str in args["variables"]:
                if var_str == "sp_z":
                    if pressure_level == "surface":
                        bins = np.arange(450.0, 1080.1)
                    elif pressure_level == "700":
                        bins = np.arange(150.0, 450.1)
                    elif pressure_level == "500":
                        bins = np.arange(300.0, 700.1)
                    elif pressure_level == "300":
                        bins = np.arange(700.0, 1400.1)
                else:
                    bins = BINS[var_str]
                ...
                pressure_level = (
                    int(pressure_level)
                    if isinstance(pressure_level, float)
                    else pressure_level
                )
                ...
                bincount_file = "%s/%s_bincounts_%d%02d.nc" % (
                    output_folder,
                    save_var_str,
                    year,
                    month,
                ...
                if bincount_ds_exists and not args["overwrite"]:
                    print(
                        "%s already exists. If you want to overwrite the existing dataset, rerun this script with the "
                ...
                for ilat in range(Nlat):
                    bincounts_by_latitude[ilat, :] += np.histogram(var[ilat, :], bins)[
                        0
                    ]
                del var
                ...
                bincount_ds = xr.Dataset(
                    data_vars={
                        "%s_bincount" % save_var_str: (
                            ("latitude", "bin"),
                            bincounts_by_latitude,
                        )
                    },
                    coords={"latitude": lat, "bin": bins[:-1]},
                )
                bincount_ds = bincount_ds.expand_dims(
                    {"time": np.array(["%d-%02d" % (year, month)], dtype=datetime)}
                )
        ...
        for var_str in args["variables"]:
            ...
            bincount_ds = bincount_ds.expand_dims(
                {"time": np.array(["%d-%02d" % (year, month)], dtype=datetime.datetime)}
            )
            bincount_ds.to_netcdf(bincount_file, engine="netcdf4", mode="w")
            bincount_ds.close()
