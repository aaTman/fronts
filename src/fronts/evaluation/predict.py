...
def _add_image_to_map(
    ...
    if lon_image == 0:  # If the image is on the western edge of the domain
        if lat_image == 0:  # If the image is on the northern edge of the domain
            ...
            stitched_map_probs[:, 0:image_size_lon, 0:image_size_lat] = image_probs[
                :, :image_size_lon, :image_size_lat
            ]
            ...
            lat_image != num_images_lat - 1
        ):  # If the image is not on the northern nor the southern edge of the domain
            # Take the maximum of the overlapping pixels along sets of constant latitude
            stitched_map_probs[
                :,
                0:image_size_lon,
                int(lat_image * lat_image_spacing) : int(
                    (lat_image - 1) * lat_image_spacing
                )
                + image_size_lat,
            ] = np.maximum(
                stitched_map_probs[
            ...
            stitched_map_probs[
                :,
                0:image_size_lon,
                int(lat_image_spacing * (lat_image - 1)) + image_size_lat : int(
                    lat_image_spacing * lat_image
                )
                + image_size_lat,
            ] = image_probs[
                :, :image_size_lon, image_size_lat - lat_image_spacing : image_size_lat
            ]
            ...
            stitched_map_probs[
                :,
                0:image_size_lon,
                int(lat_image_spacing * (lat_image - 1)) + image_size_lat :,
            ] = image_probs[
                :, :image_size_lon, image_size_lat - lat_image_spacing : image_size_lat
            ]
        ...
        lon_image != num_images_lon - 1
    ):  # If the image is not on the western nor the eastern edge of the domain
        if lat_image == 0:  # If the image is on the northern edge of the domain
            # Take the maximum of the overlapping pixels along sets of constant longitude
            stitched_map_probs[
                :,
                int(lon_image * lon_image_spacing) : int(
                    (lon_image - 1) * lon_image_spacing
                )
                + image_size_lon,
                0:image_size_lat,
            ] = np.maximum(
            ...
            stitched_map_probs[
                :,
                int(lon_image_spacing * (lon_image - 1))
                + image_size_lon : lon_image_spacing * lon_image + image_size_lon,
                0:image_size_lat,
            ] = image_probs[
                :, image_size_lon - lon_image_spacing : image_size_lon, :image_size_lat
            ]
            ...
            stitched_map_probs[
                :,
                int(lon_image * lon_image_spacing) : int(
                    lon_image_spacing * (lon_image - 1)
                )
                + image_size_lon,
                int(lat_image * lat_image_spacing) : int(lat_image * lat_image_spacing)
                + image_size_lat,
            ] = np.maximum(
                stitched_map_probs[
                    :,
            ...
            stitched_map_probs[
                :,
                int(lon_image * lon_image_spacing) : int(lon_image * lon_image_spacing)
                + image_size_lon,
                int(lat_image * lat_image_spacing) : int(
                    lat_image_spacing * (lat_image - 1)
                )
                + image_size_lat,
            ] = np.maximum(
                stitched_map_probs[
            ...
            stitched_map_probs[
                ...
                :,
                image_size_lon - lon_image_spacing : image_size_lon,
                image_size_lat - lat_image_spacing : image_size_lat,
            ]
            ...
            stitched_map_probs[
                ...
                + image_size_lon,
                int(lat_image * lat_image_spacing) :,
            ] = np.maximum(
                stitched_map_probs[
                    :,
                    int(lon_image * lon_image_spacing) : int(
        ...
        if lat_image == 0:  # If the image is on the northern edge of the domain
            ...
            stitched_map_probs[
                :,
                int(lon_image_spacing * (lon_image - 1)) + image_size_lon :,
                0:image_size_lat,
            ] = image_probs[
                :, image_size_lon - lon_image_spacing : image_size_lon, :image_size_lat
            ]
...
def find_matches_for_domain(
    domain_size: tuple | list,
    image_size: tuple | list,
    compatibility_mode: bool = False,
    compat_images: tuple | list = None,
):
    """
    ...
    if not isinstance(domain_size, (tuple, list)):
        raise TypeError(
            f"Expected a tuple or list for domain_size, received {type(domain_size)}"
        )
    elif len(domain_size) != 2:
        raise TypeError(
            f"Tuple or list for num_images must be length 2, received length {len(domain_size)}"
        )

    if not isinstance(image_size, (tuple, list)):
        raise TypeError(
            f"Expected a tuple or list for image_size, received {type(image_size)}"
        )
    elif len(image_size) != 2:
        raise TypeError(
            f"Tuple or list for image_size must be length 2, received length {len(image_size)}"
        )

    if compatibility_mode is not None and not isinstance(compatibility_mode, bool):
        raise TypeError(
            f"compatibility_mode must be a boolean, received {type(compatibility_mode)}"
        )

    if compat_images is not None:
        if not isinstance(compat_images, (tuple, list)):
            raise TypeError(
                f"Expected a tuple or list for compat_images, received {type(compat_images)}"
            )
        elif len(compat_images) != 2:
            raise TypeError(
                f"Tuple or list for compat_images must be length 2, received length {len(compat_images)}"
            )
    ####################################################################################################################

    if compatibility_mode:
        """ These parameters are used when checking the compatibility of image stitching arguments. """
        compat_images_lon = compat_images[
            0
        ]  # Number of images in the longitude direction
        compat_images_lat = compat_images[
            1
        ]  # Number of images in the latitude direction
    else:
    ...
    num_matches = [
        0,
        0,
    ]  # Total number of matching image arguments found for each dimension

    lon_image_matches = []
    lat_image_matches = []

    for lon_images in range(
        1, domain_size[0] - image_size[0] + 2
    ):  # Image counter for longitude dimension
        if lon_images > 1:
            lon_spacing = (domain_size[0] - image_size[0]) / (
                lon_images - 1
            )  # Spacing between images in the longitude dimension
        else:
            lon_spacing = 0
        if (
            lon_spacing - int(lon_spacing) == 0
            and lon_spacing > 1
            and image_size[0] - lon_spacing > 0
        ):  # Check compatibility of latitude image spacing
            lon_image_matches.append(lon_images)  # Add longitude image match to list
            num_matches[0] += 1
            if compatibility_mode:
                if (
                    compat_images_lon == lon_images
                ):  # If the number of images for the compatibility check equals the match
    ...
    if num_matches[0] == 0:
        raise ValueError(
            f"No compatible value for num_images[0] was found with domain_size[0]={domain_size[0]} and image_size[0]={image_size[0]}."
        )
    if compatibility_mode:
        if not lon_images_are_compatible:
            raise ValueError(
                f"num_images[0]={compat_images_lon} is not compatible with domain_size[0]={domain_size[0]} "
    ...
    for lat_images in range(
        1, domain_size[1] - image_size[1] + 2
    ):  # Image counter for latitude dimension
        if lat_images > 1:
            lat_spacing = (domain_size[1] - image_size[1]) / (
                lat_images - 1
            )  # Spacing between images in the latitude dimension
        else:
            lat_spacing = 0
        if (
            lat_spacing - int(lat_spacing) == 0
            and lat_spacing > 1
            and image_size[1] - lat_spacing > 0
        ):  # Check compatibility of latitude image spacing
            lat_image_matches.append(lat_images)  # Add latitude image match to list
            num_matches[1] += 1
            if compatibility_mode:
                if (
                    compat_images_lat == lat_images
                ):  # If the number of images for the compatibility check equals the match
    ...
    if num_matches[1] == 0:
        raise ValueError(
            f"No compatible value for num_images[1] was found with domain_size[1]={domain_size[1]} and image_size[1]={image_size[1]}."
        )
    if compatibility_mode:
        if not lat_images_are_compatible:
            raise ValueError(
                f"num_images[1]={compat_images_lat} is not compatible with domain_size[1]={domain_size[1]} "
...
def create_model_prediction_dataset(
    stitched_map_probs: np.array,
    lats: np.array,
    lons: np.array,
    front_types: str | list,
):
    """
    ...
    if not isinstance(stitched_map_probs, np.ndarray):
        raise TypeError(
            f"stitched_map_probs must be a NumPy array, received {type(stitched_map_probs)}"
        )
    ...
    if not isinstance(front_types, (tuple, list)):
        raise TypeError(
            f"Expected a tuple or list for front_types, received {type(front_types)}"
        )
    ####################################################################################################################
    ...
    if (
        front_types == "F_BIN"
        or front_types == "MERGED-F_BIN"
        or front_types == "MERGED-T"
    ):
        probs_ds = xr.Dataset(
            {front_types: (spatial_dims, stitched_map_probs[0])}, coords=coords
        )
        ...
        for probs_ds_index, front_type in enumerate(front_types):
            probs_ds_dict[front_type] = (
                spatial_dims,
                stitched_map_probs[probs_ds_index],
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mergir_indir",
        type=str,
        help="Input directory for the netCDF files containing MERGIR data.",
    )
    parser.add_argument(
        "--init_time",
        type=int,
        nargs=4,
        help="Date and time of the data. Pass 4 ints in the following order: year, month, day, hour",
    )
    ...
    if len(gpus) > 0:
        tf.config.set_visible_devices(
            devices=gpus[args["gpu_device"]], device_type="GPU"
        )
        if args["memory_growth"]:
            tf.config.experimental.set_memory_growth(
                device=gpus[args["gpu_device"]], enable=True
            )
    ...
    model_properties = pd.read_pickle(
        f"{args['model_dir']}/model_{args['model_number']}/model_{args['model_number']}_properties.pkl"
    )
    model_type = model_properties["model_type"]

    if args["image_size"] is None:
        args["image_size"] = model_properties[
            "image_size"
        ]  # The image size does not include the last dimension of the input size as it only represents the number of channels
    ...
    load_mergir = (
        "Tb" in variables
    )  # load MERGIR data if brightness temperature (Tb) is requested
    ...
    test_years, valid_years = (
        model_properties["test_years"],
        model_properties["validation_years"],
    )
    ...
    image_size_lon, image_size_lat = args[
        "image_size"
    ]  # Dimensions of the model's predictions

    if num_images_lon > 1:
        lon_image_spacing = int(
            (domain_size_lon - image_size_lon) / (num_images_lon - 1)
        )
    else:
        lon_image_spacing = 0
    ...
    if num_images_lat > 1:
        lat_image_spacing = int(
            (domain_size_lat - image_size_lat) / (num_images_lat - 1)
        )
    else:
        lat_image_spacing = 0
    ...
    ############################################### Load variable files ################################################
    variable_files_obj = fm.DataFileLoader(
        args["netcdf_indir"],
        data_type=args["data_source"],
        file_format="netcdf",
        years=int(args["init_time"][0]),
        months=int(args["init_time"][1]),
        days=int(args["init_time"][2]),
        hours=int(args["init_time"][3]),
    )

    if load_mergir:
        variable_files_obj.add_file_list(
            args["mergir_indir"], "MERGIR", ignore_domain=True
        )
    ...
    dataset_kwargs = {
        "engine": "netcdf4"
    }  # Keyword arguments for loading variable files with xarray
    ...
    if args["init_time"] is not None:
        ...
        if args["data_source"] == "era5":
            init_time_index = [
                index
                for index, file in enumerate(variable_files)
                if timestep_str in file
            ][0]
            variable_files = [
                variable_files[init_time_index],
            ]
        else:
    ...
    variable_ds = xr.open_mfdataset(variable_files, **dataset_kwargs).sel(
        **coords_sel_kwargs
    )[variables]
    if load_mergir:
        mergir_ds = xr.open_mfdataset(mergir_files, **dataset_kwargs).rename(
        ...
        mergir_dataset = mergir_dataset.reindex(
            longitude=sorted(mergir_lons), latitude=mergir_lats[::-1]
        )
        mergir_dataset = (
            mergir_dataset.isel(time=0)
            .transpose("longitude", "latitude")
            .sel(**coords_sel_kwargs)
        )
        ...
        Tb = scipy.interpolate.RegularGridInterpolator(
            (new_mergir_lons, new_mergir_lats), Tb, method="nearest", bounds_error=False
        )((new_lons, new_lats)).transpose()[..., np.newaxis]
        Tb = (Tb - 197) / (329 - 197)  # min max normalization
        Tb = np.nan_to_num(Tb).transpose()

    if args["data_source"] == "era5":
        variable_ds = variable_ds.sel(pressure_level=pressure_levels).transpose(
            *transpose_dims
        )
        ...
        if args["data_source"] in ["hrrr", "nam-12km"]:
            ...
            image_lons = variable_ds.longitude.values[
                :domain_size_lon, :domain_size_lat
            ]
        else:
            image_lats = variable_ds.latitude.values[:domain_size_lat]
            image_lons = variable_ds.longitude.values[:domain_size_lon]
    ...
    map_created = (
        False  # Boolean that determines whether the final stitched map has been created
    )

    if args["data_source"] == "era5":
        stitched_map_probs = np.empty(
            shape=[num_timesteps, classes - 1, domain_size_lon, domain_size_lat]
        )
    else:
        stitched_map_probs = np.empty(
            shape=[
    ...
    for lat_image in range(num_images_lat):
        ...
        for lon_image in range(num_images_lon):
            print(
                f"image %d/%d"
                % (
                    int(lat_image * num_images_lon) + lon_image + 1,
                    int(num_images_lon * num_images_lat),
                )
            )
            lon_index = int(lon_image * lon_image_spacing)

            # Select the current image
            variable_batch_ds_new = (
                variable_batch_ds[variables]
                .isel(
                    {
                        "%s" % spatial_dims[0]: slice(
                            lon_index, lon_index + args["image_size"][0]
                        ),
                        "%s" % spatial_dims[1]: slice(
                            lat_index, lat_index + args["image_size"][1]
                        ),
                    }
                )
                .to_array()
            ...
            if load_mergir:
                assert args["num_images"] == [1, 1], (
                    "Can only use MERGIR data if making a prediction using one image only."
                )

                Tb = Tb[
                    np.newaxis, ...
                ]  # add 'variable' dimension so it can be concatenated
                if args["data_source"] != "era5":
                ...
                if num_dimensions == 3:
                    ...
                    Tb = np.tile(
                        Tb,
                        [
                            *[1 for dim in range(len(Tb.shape) - 3)],
                            len(pressure_levels),
                            1,
                            1,
                        ],
                    )
                variable_batch_ds_new = np.concatenate(
                    [variable_batch_ds_new, Tb], axis=0
                )

            if args["data_source"] == "era5":
                variable_batch_ds_new = variable_batch_ds_new.transpose(
                    [1, 2, 3, 4, 0]
                )  # (time, longitude, latitude, pressure level, variable)
            else:
                variable_batch_ds_new = variable_batch_ds_new.transpose(
                    [1, 2, 3, 4, 5, 0]
                )  # (time, forecast hour, longitude, latitude, pressure level, variable)

            if num_dimensions == 2:
                ### Combine pressure levels and variables into one dimension ###
                variable_batch_ds_new_shape = np.shape(variable_batch_ds_new)
                variable_batch_ds_new = variable_batch_ds_new.reshape(
                    *[dim_size for dim_size in variable_batch_ds_new_shape[:-2]],
                    variable_batch_ds_new_shape[-2] * variable_batch_ds_new_shape[-1],
                )
            ...
            transpose_indices = (
                ...
                1,
                2,
            )  # New order of indices for model predictions (time, front type, longitude, latitude)

            ##################################### Generate the predictions #####################################
            if args["data_source"] != "era5":
                variable_ds_new_shape = np.shape(variable_batch_ds_new)
                variable_batch_ds_new = variable_batch_ds_new.reshape(
                    variable_ds_new_shape[0] * variable_ds_new_shape[1],
                    *[dim_size for dim_size in variable_ds_new_shape[2:]],
                )

            prediction = model.predict(
                variable_batch_ds_new, batch_size=args["batch_size"], verbose=0
            )
            ...
            if model_type in ["unet", "attention_unet"]:
                if (
                    num_dims_in_pred == 4
                ):  # 2D labels, prediction shape: (time, lat, lon, front type)
                    image_probs = np.transpose(
                        prediction[:, :, :, 1:], transpose_indices
                    )  # transpose the predictions
                else:  # if num_dims_in_pred == 5; 3D labels, prediction shape: (time, lat, lon, pressure level, front type)
                    image_probs = np.transpose(
                        np.amax(prediction[:, :, :, :, 1:], axis=3), transpose_indices
                    )  # Take the maximum probability over the vertical dimension and transpose the predictions
                ...
                if deep_supervision:
                    if (
                        num_dims_in_pred == 5
                    ):  # 2D labels, prediction shape: (output level, time, lon, lat, front type)
                        image_probs = np.transpose(
                            prediction[0][:, :, :, 1:], transpose_indices
                        )  # transpose the predictions
                    else:  # if num_dims_in_pred == 6; 3D labels, prediction shape: (output level, time, lon, lat, pressure level, front type)
                        image_probs = np.transpose(
                            np.amax(prediction[0][:, :, :, :, 1:], axis=3),
                            transpose_indices,
                        )  # Take the maximum probability over the vertical dimension and transpose the predictions
                else:
                    if (
                        num_dims_in_pred == 4
                    ):  # 2D labels, prediction shape: (time, lon, lat, front type)
                        image_probs = np.transpose(
                            prediction[:, :, :, 1:], transpose_indices
                        )  # transpose the predictions
                    else:  # if num_dims_in_pred == 5; 3D labels, prediction shape: (time, lat, lon, pressure level, front type)
                        image_probs = np.transpose(
                            np.amax(prediction[:, :, :, :, 1:], axis=3),
                            transpose_indices,
                        )  # Take the maximum probability over the vertical dimension and transpose the predictions
                ...
                for fcst_hr_index in range(num_forecast_hours):
                    if args["num_images"] == [1, 1]:
                        ...
                        if (
                            ...
                            and fcst_hr_index == num_forecast_hours - 1
                        ):
                            map_created = True
                    else:
                        stitched_map_probs[timestep][fcst_hr_index], map_created = (
                            _add_image_to_map(
                                stitched_map_probs[timestep][fcst_hr_index],
                                image_probs[
                                    timestep * num_forecast_hours + fcst_hr_index
                                ],
                                map_created,
                                num_images_lon,
                                num_images_lat,
            ...
            if map_created:
                for timestep_no, timestep in enumerate(timesteps):
                    ...
                    for fcst_hr_index, forecast_hour in enumerate(forecast_hours):
                        time = f"{timestep[:4]}%s%s%s" % (
                            timestep[5:7],
                            timestep[8:10],
                            timestep[11:13],
                        )
                        probs_ds = create_model_prediction_dataset(
                            stitched_map_probs[timestep_no][fcst_hr_index],
                            image_lats,
                            image_lons,
                            front_types,
                        )
                        probs_ds = probs_ds.expand_dims(
                            {
                                "time": np.atleast_1d(timestep),
                                "forecast_hour": np.atleast_1d(
                                    forecast_hours[fcst_hr_index]
                                ),
                            }
                        )
                        filename_base = "model_%d_%s_%s_%s_f%03d" % (
                            args["model_number"],
                            time,
                            args["domain"],
                            args["data_source"],
                        ...
                        pred_folder = "%s/model_%d/predictions" % (
                            args["model_dir"],
                            args["model_number"],
                        )
                        os.makedirs(pred_folder, exist_ok=True)
                        outfile = "%s/%s_probabilities.nc" % (
                            pred_folder,
                            filename_base,
                        )
