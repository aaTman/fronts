...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="test",
        help="Dataset for which to make saliency maps. Options are: 'training', 'validation', 'test'",
    )
    parser.add_argument(
        "--year_and_month",
        type=int,
        nargs=2,
        help="Year and month for which to make saleincy maps.",
    )
    parser.add_argument(
        "--tf_indir",
        type=str,
    ...
    parser.add_argument(
        "--freq", type=str, required=True, help="Timestep frequency of the input data."
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Print out the progress of saliency map generation by batch.",
    )
    ...
    dataset_properties = pd.read_pickle(
        "%s/dataset_properties.pkl" % args["tf_indir"]
    )  # properties of the dataset being used for saliency maps
    ...
    for year in years:
        ...
        for month in months:
            ...
            try:
                tf_ds = tf.data.Dataset.load(
                    [
                        file
                        for file in files_for_year
                        if "_%d%02d" % (year, month) in file
                    ][0]
                )
            except IndexError:
                print(
                    "ERA5 tensorflow dataset not found for %d-%02d in %s"
                    % (year, month, args["tf_indir"])
                )
                continue
            ...
            init_times = pd.date_range(
                "%s-%02d" % (year, month),
                "%s-%02d" % (next_year, next_month),
                freq=args["freq"],
            )[:-1]

            os.makedirs(
                "%s/model_%d/saliencymaps" % (args["model_dir"], args["model_number"]),
                exist_ok=True,
            )  # make directory for the saliency maps
            salmap_dataset_path = (
                "%s/model_%d/saliencymaps/model_%d_salmap_%s_%d%02d.nc"
                % (
                    args["model_dir"],
                    args["model_number"],
                    args["model_number"],
            ...
            assert len(tf_ds) == len(init_times), (
                "Length of provided tensorflow dataset (%d) does not match the number of timesteps (%d) in %d-%02d with the provided frequency (%s)"
                % (len(tf_ds), len(init_times), year, month, args["freq"])
            )

            tf_ds = tf_ds.batch(
                args["batch_size"]
            )  # split dataset into batches (necessary for saliency maps because of memory issues)
            ...
            for batch_num, batch in enumerate(tf_ds, start=1):
                ...
                with tf.GradientTape(persistent=True) as tape:
                    ...
                    batch_gradient = np.stack(
                        [
                            np.max(
                                tape.gradient(
                                    predictions[..., class_idx + 1 : class_idx + 2],
                                    batch,
                                ).numpy(),
                                axis=-1,
                            )
                    ...
                    gradients = (
                        ...
                        else np.concatenate([gradients, batch_gradient], axis=0)
                    )

            domain_ext = data_utils.DOMAIN_EXTENTS[domain]

            domain_size = (
                int((domain_ext[1] - domain_ext[0]) // 0.25) + 1,
                int((domain_ext[3] - domain_ext[2]) // 0.25) + 1,
            )

            lons = np.linspace(domain_ext[0], domain_ext[1], domain_size[0])
            lats = np.linspace(domain_ext[2], domain_ext[3], domain_size[1])[
                ::-1
            ]  # lats in descending order (north-south)
                ...
                data_vars=dict(
                    {
                        "%s" % front_type: (
                            ...
                            np.max(gradients[..., idx], axis=-1),
                        )
                        for idx, front_type in enumerate(front_types)
                    }
                    | {
                        "%s_pl" % front_type: (
                            ("time", "longitude", "latitude", "pressure_level"),
                            gradients[..., idx],
                        )
                        for idx, front_type in enumerate(front_types)
                    }
