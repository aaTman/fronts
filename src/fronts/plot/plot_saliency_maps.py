...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--plot_outdir", type=str, help="Directory for the saliency maps."
    )
    parser.add_argument(
        "--init_time",
        type=str,
        help="Initialization time of the data. Format: YYYY-MM-DD-HH.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="viridis",
        help="Colormap to use for the saliency maps.",
    )
    parser.add_argument(
        "--extent",
        type=int,
    ...
    extent = (
        data_utils.DOMAIN_EXTENTS[args["domain"]]
    ...
    salmap_folder = "%s/model_%d/saliencymaps" % (
        args["model_dir"],
        args["model_number"],
    )
    salmap_ds = xr.open_dataset(
        "%s/model_%d_salmap_%s_%s_%d%02d.nc"
        % (
            salmap_folder,
            args["model_number"],
            args["domain"],
    ...
    probs_folder = "%s/model_%d/probabilities" % (
        args["model_dir"],
        args["model_number"],
    )
    probs_ds = xr.open_dataset(
        "%s/model_%d_pred_%s_%d%02d.nc"
        % (
            probs_folder,
            args["model_number"],
            args["domain"],
    ...
    for front_type in front_types:
        if args["calibration"] is not None:
            try:
                ir_model = model_properties["calibration_models"][args["domain"]][
                    front_type
                ]["%d km" % args["calibration"]]
            except KeyError:
                ir_model = model_properties["calibration_models"]["conus"][front_type][
                    "%d km" % args["calibration"]
                ]
            original_shape = np.shape(probs_ds[front_type].values)
            probs_ds[front_type].values = ir_model.predict(
                probs_ds[front_type].values.flatten()
        ...
        probs_ds[front_type].values = np.where(
            probs_ds[front_type].values < 0.1, np.nan, probs_ds[front_type].values
        )

        cmap_probs, norm = (
            cm.get_cmap(data_utils.CONTOUR_CMAPS[front_type], 11),
        ...
        salmap_for_type = (salmap_for_type - min_gradient) / (
            max_gradient - min_gradient
        )
        salmap_for_type_pl = (salmap_for_type_pl - min_gradient) / (
            max_gradient - min_gradient
        )
        ...
        for ax_ind, ax in enumerate(axarr):
            ...
            if ax_ind == 0:
                probs_ds[front_type].plot.contourf(
                    ax=ax,
                    x="longitude",
                    y="latitude",
                    cmap=cmap_probs,
                ...
                salmap_for_type_pl.isel(pressure_level=ax_ind - 1).plot(
                    ax=ax,
                    x="longitude",
                    y="latitude",
                    cmap=args["cmap"],
        ...
        cbar = plt.colorbar(
            cm.ScalarMappable(
                norm=matplotlib.colors.Normalize(vmin=0, vmax=1), cmap=args["cmap"]
            ),
        ...
        plot_outdir = (
            args["plot_outdir"] if args["plot_outdir"] is not None else salmap_folder
        )

        plt.tight_layout()
        plt.suptitle(
            "%s predictions: %d-%02d-%02d-%02dz"
            % (
                data_utils.FRONT_NAMES[front_type],
                init_time.year,
                init_time.month,
                init_time.day,
                init_time.hour,
        ...
        plt.savefig(
            "%s/model_%d_salmap_%d%02d%02d%02d_%s_%s.png"
            % (
                plot_outdir,
                args["model_number"],
                init_time.year,
