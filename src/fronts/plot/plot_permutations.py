...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--domain", type=str, required=True, help="Domain for the permutations"
    )
    parser.add_argument(
        "--show_names",
        action="store_true",
        help="Show variable and level names instead of prefixes.",
    )
    parser.add_argument(
    ...
    model_properties = pd.read_pickle(
        "%s/model_%d_properties.pkl" % (model_folder, args["model_number"])
    )
    ...
    permutations_dict = pd.read_pickle(
        "%s/permutations_%d_%s.pkl"
        % (model_folder, args["model_number"], args["domain"])
    )  # contains permutation data
    ...
    sp_vars = np.array(
        [sp[var] for var in variables]
    )  # single-pass results: shuffled variables over all levels
    sp_lvls = np.array(
        [sp[lvl] for lvl in pressure_levels]
    )  # single-pass results: shuffled levels over all variables
    sp_sorting_orders = [
        np.argsort(sp_vars, axis=0),
        np.argsort(sp_lvls, axis=0),
    ]  # list of length 2: sorted grouped variables and grouped levels

    for front_num, front_type in enumerate(front_types):
        sp_sorted_data = [
            sp_vars[:, front_num][sp_sorting_orders[0][:, front_num]],
            sp_lvls[:, front_num][sp_sorting_orders[1][:, front_num]],
        ]
        sp_sorted_vars_and_lvls = [
            [variables[ind] for ind in sp_sorting_orders[0][:, front_num]],
            [pressure_levels[ind] for ind in sp_sorting_orders[1][:, front_num]],
        ]

        y_pos_vars = np.arange(
            num_vars
        )  # array marking the y-positions of the variables
        y_pos_lvls = np.arange(
            num_lvls
        )  # array marking the y-positions of the pressure levels

        fig = plt.figure(figsize=(12, 9))
        ax1 = plt.subplot(
            2, 2, 1
        )  # axis for the subplot containing single-pass variable permutations
        ax2 = plt.subplot(
            2, 2, 2
        )  # axis for the subplot containing single-pass level permutations
        ...
        for pos, data in enumerate(sp_sorted_data[0]):
            ax1.annotate(
                ...
                xy=(
                    np.max([data, 0])
                    + (np.max(sp_sorted_data[0]) * x_margin_adjust / 10),
                    pos,
                ),
                va="center",
            )
        for pos, data in enumerate(sp_sorted_data[1]):
            ax2.annotate(
                ...
                xy=(
                    np.max([data, 0])
                    + (np.max(sp_sorted_data[1]) * x_margin_adjust / 10),
                    pos,
                ),
                va="center",
            )
        ...
        for ax in [ax1, ax2]:
            ax.margins(
                x=x_margin_adjust
            )  # increase the scale of the x-axis so text will not extend past the right side of the subplots
        ...
        cellValues = np.array(
            [
                [sp["_".join([var, lvl])][front_num] for lvl in pressure_levels]
        ...
        cellText = np.array(
            [
                "%d (%.3f)" % (rank, val)
                for rank, val in zip(ranks, cellValues.flatten())
            ]
        ).reshape(cellValues.shape)
        ...
        rowColours = [
            "gray" for _ in range(len(variables))
        ]  # shade first column containing variable names
        colColours = [
            "gray" for _ in range(len(pressure_levels))
        ]  # shade header cells containing pressure level names
