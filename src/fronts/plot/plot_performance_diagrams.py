...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, help="'training', 'validation', or 'test'"
    )
    parser.add_argument(
        "--data_source",
        type=str,
        default="era5",
        help="Source of the variable data (ERA5, GDAS, etc.)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=95,
        help="Confidence level expressed as a percentage.",
    )
    parser.add_argument(
        "--num_iterations",
        type=int,
        default=10000,
        help="Number of iterations to perform when bootstrapping the statistics.",
    )
    parser.add_argument(
        "--output_type", type=str, default="png", help="Output type for the image file."
    )
    ...
    spatial_files = list(
        sorted(
            glob(
                "%s/model_%d/statistics/model_%d_statistics_%s_*_spatial.nc"
                % (
                    args["model_dir"],
                    args["model_number"],
                    args["model_number"],
    ...
    temporal_files = list(
        sorted(
            glob(
                "%s/model_%d/statistics/model_%d_statistics_%s_*_temporal.nc"
                % (
                    args["model_dir"],
                    args["model_number"],
                    args["model_number"],
    ...
    for front_type in front_types:
        ...
        for iteration in range(args["num_iterations"]):
            ...
            indices = random.choices(
                selectable_indices, k=num_timesteps
            )  # Select a sample equal to the total number of timesteps
            tp_array[iteration, :, :] = np.sum(
                true_positives_temporal[indices, :, :], axis=0
            )
            fp_array[iteration, :, :] = np.sum(
                false_positives_temporal[indices, :, :], axis=0
            )
            tn_array[iteration, :, :] = np.sum(
                true_negatives_temporal[indices, :, :], axis=0
            )
            fn_array[iteration, :, :] = np.sum(
                false_negatives_temporal[indices, :, :], axis=0
            )
        ...
        for percent in np.arange(0, 100):
            ...
            CI_lower_tp[:, percent] = np.percentile(
                tp_array[:, :, percent], q=lower_bound, axis=0
            )
            CI_lower_fp[:, percent] = np.percentile(
                fp_array[:, :, percent], q=lower_bound, axis=0
            )
            CI_lower_tn[:, percent] = np.percentile(
                tn_array[:, :, percent], q=lower_bound, axis=0
            )
            CI_lower_fn[:, percent] = np.percentile(
                fn_array[:, :, percent], q=lower_bound, axis=0
            )

            # lower bound for confidence intervals
            CI_upper_tp[:, percent] = np.percentile(
                tp_array[:, :, percent], q=upper_bound, axis=0
            )
            CI_upper_fp[:, percent] = np.percentile(
                fp_array[:, :, percent], q=upper_bound, axis=0
            )
            CI_upper_tn[:, percent] = np.percentile(
                tn_array[:, :, percent], q=upper_bound, axis=0
            )
            CI_upper_fn[:, percent] = np.percentile(
                fn_array[:, :, percent], q=upper_bound, axis=0
            )
        ...
        CI_lower_HSS = (
            2
            * ((CI_lower_tp * CI_lower_tn) - (CI_upper_fp * CI_upper_fn))
            / (
                ((CI_lower_tp + CI_upper_fn) * (CI_upper_fn + CI_lower_tn))
                + ((CI_lower_tp + CI_upper_fp) * (CI_upper_fp + CI_lower_tn))
            )
        )
        CI_upper_HSS = (
            2
            * ((CI_upper_tp * CI_upper_tn) - (CI_lower_fp * CI_lower_fn))
            / (
                ((CI_upper_tp + CI_lower_fn) * (CI_lower_fn + CI_upper_tn))
                + ((CI_upper_tp + CI_lower_fp) * (CI_lower_fp + CI_upper_tn))
            )
        )

        CI_POD = np.stack((CI_lower_POD, CI_upper_POD), axis=0)
        CI_SR = np.stack((CI_lower_SR, CI_upper_SR), axis=0)
        CI_CSI = np.stack((CI_SR**-1 + CI_POD**-1 - 1.0) ** -1, axis=0)
        ...
        spatial_csi_ds = (
            ...
            / (
                ...
                + spatial_ds[f"fn_spatial_{front_type}"].sum("time")
            )
        ).max("threshold")
        ...
        total_pixels = (
            true_positives_temporal.shape[0]
            * len(spatial_csi_ds["latitude"])
            * len(spatial_csi_ds["longitude"])
        )
        ...
        observed_relative_frequency = np.divide(
            true_positives_diff, true_positives_diff + false_positives_diff
        )

        pod = np.divide(
            true_positives_temporal_sum,
            true_positives_temporal_sum + false_negatives_temporal_sum,
        )  # Probability of detection
        sr = np.divide(
            true_positives_temporal_sum,
            true_positives_temporal_sum + false_positives_temporal_sum,
        )  # Success ratio

        fig, axs = plt.subplots(1, 2, figsize=(15, 6))
        axarr = axs.flatten()

        sr_matrix, pod_matrix = np.meshgrid(
        ...
        csi_contour = axarr[0].contourf(
            sr_matrix, pod_matrix, csi_matrix, CSI_LEVELS, cmap=cmap
        )  # Plot CSI contours in 0.1 increments
        cbar = fig.colorbar(
            csi_contour, ax=axarr[0], pad=0.02, label="Critical Success Index (CSI)"
        )
        ...
        for boundary, color in enumerate(boundary_colors):
            ...
            cell_text.append(
                [
                    r"$\bf{%.3f}$" % max_CSI_scores_by_boundary[boundary]
                    + r"$^{%.3f}_{%.3f}$"
                    % (
                        CI_CSI[1, boundary, max_CSI_index][0],
                        CI_CSI[0, boundary, max_CSI_index][0],
                    ),
                    r"$\bf{%.3f}$" % max_HSS_scores_by_boundary[boundary, max_CSI_index]
                    + r"$^{%.3f}_{%.3f}$"
                    % (
                        CI_HSS[1, boundary, max_CSI_index][0],
                        CI_HSS[0, boundary, max_CSI_index][0],
                    ),
                    r"$\bf{%.1f}$" % (max_CSI_pod * 100)
                    + r"$^{%.1f}_{%.1f}$"
                    % (
                        CI_POD[1, boundary, max_CSI_index][0] * 100,
                        CI_POD[0, boundary, max_CSI_index][0] * 100,
                    ),
                    r"$\bf{%.1f}$" % ((1 - max_CSI_sr) * 100)
                    + r"$^{%.1f}_{%.1f}$"
                    % (
                        (1 - CI_SR[1, boundary, max_CSI_index][0]) * 100,
                        (1 - CI_SR[0, boundary, max_CSI_index][0]) * 100,
                    ),
                    r"$\bf{%.3f}$" % max_CSI_fb
                    + r"$^{%.3f}_{%.3f}$"
                    % (
                        CI_FB[1, boundary, max_CSI_index][0],
                        CI_FB[0, boundary, max_CSI_index][0],
                    ),
                ]
            )
            ...
            # Plot CSI lines
            axarr[0].plot(
                max_CSI_sr, max_CSI_pod, color=color, marker="*", markersize=10
            )
            ...
            axarr[1].plot(
                thresholds[:-1],
                observed_relative_frequency[boundary],
                color=color,
                linewidth=1,
            )
            ...
            xs = np.concatenate(
                [
                    CI_SR[0, boundary, :polygon_stop_index],
                    CI_SR[1, boundary, :polygon_stop_index][::-1],
                ]
            )
            ys = np.concatenate(
                [
                    CI_POD[0, boundary, :polygon_stop_index],
                    CI_POD[1, boundary, :polygon_stop_index][::-1],
                ]
            )
            axarr[0].fill(
                xs, ys, alpha=0.3, color=color
            )  # Shade the confidence interval

        axarr[0].set_xticklabels(
            axis_ticklabels[::-1]
        )  # False alarm rate on x-axis means values are reversed
        axarr[0].set_xlabel("False Alarm Rate (FAR; %)")
        axarr[0].set_ylabel("Probability of Detection (POD; %)")
        axarr[0].set_title(
            r"$\bf{a)}$ $\bf{CSI}$ $\bf{diagram}$ [confidence level = %d%%]"
        ...
        cbar_kwargs = {
            "label": "CSI",
            "pad": 0,
        }  # Spatial CSI colorbar keyword arguments
        ...
        if args["domain"] == "conus":
            ...
            raise ValueError(
                "%s domain is currently not supported for performance diagrams."
                % args["domain"]
            )
        ...
        table_axis.set_title(
            r"$\bf{c)}$ $\bf{Data}$ $\bf{table}$ [confidence level = %d%%]"
            % args["confidence_level"],
            **table_title_kwargs,
        )
        table_axis.axis("off")
        ...
        spatial_axis = plt.axes(
            spatial_axis_extent, projection=ccrs.Miller(central_longitude=250)
        )
        spatial_axis_title_text = (
            r"$\bf{d)}$ $\bf{%d}$ $\bf{km}$ $\bf{CSI}$ $\bf{map}$"
            % args["map_neighborhood"]
        )
        ...
        spatial_csi_ds.sel(neighborhood=args["map_neighborhood"]).plot(
            ax=spatial_axis,
            x="longitude",
            y="latitude",
            norm=norm_probs,
        ...
        gl = spatial_axis.gridlines(
            draw_labels=True, zorder=0, dms=True, x_inline=False, y_inline=False
        )
        ...
        plt.suptitle(
            f"Five-class model: %ss over %s domain"
            % (data_utils.FRONT_NAMES[front_type], domain_text),
            fontsize=20,
        )  # Create and plot the main title

        filename = (
            f"%s/model_%d/performance_%s_%s_%s_{args['data_source']}.{args['output_type']}"
            % (
                args["model_dir"],
                args["model_number"],
                front_type,
        ...
        if args["data_source"] != "era5":
            filename = filename.replace(
                f".{args['output_type']}",
                f"_f%03d.{args['output_type']}" % args["forecast_hour"],
            )  # Add forecast hour to the end of the filename
