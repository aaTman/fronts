...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Year for the netCDF data to be converted to tensorflow datasets.",
    )
    parser.add_argument(
        "--data_source",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Number of slices to perform for each year in the dataset.",
    )
    parser.add_argument(
        "--domain",
        type=str,
    ...
    )  # ensure that a folder exists for the dataset
    ...
    if not os.path.isfile(dataset_props_file):
        ...
        if "seed" in list(dataset_props.keys()):
            args["seed"] = dataset_props[
                "seed"
            ]  # keep the same seed for consistency sake and reproducibility
    ...
    # Gather all ERA5/model and front label files that can be used to generate the dataset for the current year.
    ...
    sliced_list = list(np.array_split(np.array(zipped_list), num_slices))
    ...
    for i_slice, files_in_slice in enumerate(sliced_list, start=1):
        """
        ...
        print(f"Year {yr}: Generating slice {i_slice}/{num_slices}")

        for input_file, fronts_file in files_in_slice:
            keep_timestep = (
                np.random.random() <= args["timestep_fraction"]
            )  # boolean flag for keeping timesteps without all front types

            # open front dataset
            front_dataset = xr.open_dataset(fronts_file, engine="netcdf4").isel(
                **isel_kwargs
            )
            ...
            front_dataset = front_dataset.to_array().transpose(
                *transpose_dims, "variable"
            )
            timestep_front_bins = np.bincount(
                front_dataset.values.astype("int64").flatten(),
                minlength=num_front_types,
            )  # counts for each front type
            all_fronts_in_timestep = (
                all([front_count > 0 for front_count in timestep_front_bins]) > 0
            )  # boolean flag that says if all front types are present in the current timestep
            ...
            if all_fronts_in_timestep or keep_timestep:
                ...
                if extent_crosses_meridian:
                    ...
                    variables_dataset_1 = (
                        xr.open_dataset(input_file, engine="netcdf4")[data_vars]
                    ...
                    variables_dataset = xr.merge(
                        [variables_dataset_1, variables_dataset_2]
                    )
                ...
                if (
                    args["images"][0] > 1
                    and domain_size[0] > args["image_size"][0] + args["images"][0]
                ):
                    start_indices_lat = np.linspace(
                        0, domain_size[0] - args["image_size"][0], args["images"][0]
                    ).astype(int)
                else:
                ...
                if (
                    args["images"][1] > 1
                    and domain_size[1] > args["image_size"][1] + args["images"][1]
                ):
                    start_indices_lon = np.linspace(
                        0, domain_size[1] - args["image_size"][1], args["images"][1]
                    ).astype(int)
                else:
                ...
                images_to_keep = (
                    np.random.random(size=len(image_order)) <= args["image_fraction"]
                )

                for i_image, image_start_indices in enumerate(image_order):
                    if args["verbose"]:
                        print(
                            f"{yr}: slice {i_slice}/{num_slices} (kept/discarded):  ({timesteps_kept}/{timesteps_discarded} timesteps, {images_kept}/{images_discarded} images)",
                            end="\r",
                        )
                    ...
                    image_front_bins = np.bincount(
                        front_image.values.astype("int64").flatten(),
                        minlength=num_front_types,
                    )  # counts for each front type
                    all_fronts_in_image = (
                        all([front_count > 0 for front_count in image_front_bins]) > 0
                    )  # boolean flag that says if all front types are present in the current timestep

                    if not all_fronts_in_image and not images_to_keep[i_image]:
                        images_discarded += 1
                        continue
                    ...
                    # normalize variables and convert datasets to tensors
                    new_variables_dataset = (
                    ...
                    )  # random noise values, will not necessarily be used

                    # rotate input variables image
                    if flip_lat:  # reverse values along the latitude dimension
                        input_tensor = tf.reverse(input_tensor, axis=[0])
                        front_tensor = tf.reverse(front_tensor, axis=[0])
                    if flip_lon:  # reverse values along the longitude dimension
                        input_tensor = tf.reverse(input_tensor, axis=[1])
                        front_tensor = tf.reverse(front_tensor, axis=[1])

                    # add salt and pepper noise to images
                    if args["noise_fraction"] > 0:
                        input_tensor = tf.where(
                            random_values < args["noise_fraction"] / 2,
                            0.0,
                            input_tensor,
                        )  # add 0s to image
                        input_tensor = tf.where(
                            random_values > 1.0 - (args["noise_fraction"] / 2),
                            1.0,
                            input_tensor,
                        )  # add 1s to image
                    ...
                    input_tensor_for_timestep = tf.data.Dataset.from_tensors(
                        input_tensor
                    )
                    if len(input_tensors_for_slice) == 0:
                        input_tensors_for_slice = input_tensor_for_timestep
                    else:
                        input_tensors_for_slice = input_tensors_for_slice.concatenate(
                    ...
                    # add fronts to tensorflow dataset
                    ...
                    if len(front_tensors_for_slice) == 0:
                        front_tensors_for_slice = front_tensor_for_timestep
                    else:
                        front_tensors_for_slice = front_tensors_for_slice.concatenate(
        ...
        tf_dataset_folder = (
            f"{args['tf_outdir']}/{yr}-{i_slice}_tf"  # output directory for the inputs
        )
        tf_dataset = tf.data.Dataset.zip(
            (input_tensors_for_slice, front_tensors_for_slice)
        )

        # save the tensorflow datasets
        try:
            tf.data.Dataset.save(tf_dataset, path=tf_dataset_folder)
        except NameError:
            print("No images could be retained with the provided arguments.")

    print(
        f"Tensorflow datasets for {yr} ({num_slices} slices) saved to {args['tf_outdir']}."
    )
