...
def bar_progress(current, total, width=None):
    progress_message = "Downloading %s: %d%% [%d/%d] MB       " % (
        local_filename,
        current / total * 100,
        current / 1e6,
        total / 1e6,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Output directory for GDAS grib files downloaded from NCEP.",
    )
    parser.add_argument(
        "--model", type=str, required=True, help="NWP model to use as the data source."
    )
    parser.add_argument(
        "--init_time",
        type=str,
        help="Initialization time of the model. Format: YYYY-MM-DD-HH.",
    )
    parser.add_argument(
        "--range",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="List of forecast hours to download for the given day.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Include a progress bar for download progress.",
    )
    ...
    if args["init_time"] is not None and args["range"] is not None:
        raise ValueError(
            "Only one of the following arguments can be passed: --init_time, --range"
        )
    elif args["init_time"] is None and args["range"] is None:
        raise ValueError(
            "One of the following arguments must be passed: --init_time, --range"
        )

    init_times = (
        pd.date_range(args["init_time"], args["init_time"])
        if args["init_time"] is not None
        else pd.date_range(*args["range"][:2], freq=args["range"][-1])
    )
        ...
        if args["model"] == "gdas":
            if datetime.datetime(yr, mo, dy, hr) < datetime.datetime(2015, 6, 23, 0):
                raise ConnectionAbortedError(
                    "Cannot download GDAS data prior to June 23, 2015."
                )
            elif datetime.datetime(yr, mo, dy, hr) < datetime.datetime(2017, 7, 20, 0):
                [
                    files.append(
                        "https://noaa-gfs-bdp-pds.s3.amazonaws.com/gdas.%d%02d%02d/%02d/gdas1.t%02dz.pgrb2.0p25.f%03d"
                        % (yr, mo, dy, hr, hr, forecast_hour)
                    )
                ...
                [
                    files.append(
                        "https://noaa-gfs-bdp-pds.s3.amazonaws.com/gdas.%d%02d%02d/%02d/gdas.t%02dz.pgrb2.0p25.f%03d"
                        % (yr, mo, dy, hr, hr, forecast_hour)
                    )
                    for forecast_hour in args["forecast_hours"]
                ]
                ...
                [
                    files.append(
                        "https://noaa-gfs-bdp-pds.s3.amazonaws.com/gdas.%d%02d%02d/%02d/atmos/gdas.t%02dz.pgrb2.0p25.f%03d"
                        % (yr, mo, dy, hr, hr, forecast_hour)
                    )
                    for forecast_hour in args["forecast_hours"]
                ]
            ...
            if datetime.datetime(yr, mo, dy, hr) < datetime.datetime(2017, 1, 1, 0):
                raise ConnectionAbortedError(
                    "Cannot download GEFS data prior to January 1, 2017."
                )
            elif datetime.datetime(yr, mo, dy, hr) < datetime.datetime(2018, 7, 27, 0):
                [
                    files.append(
                        "https://noaa-gefs-pds.s3.amazonaws.com/gefs.%d%02d%02d/%02d/ge%s.t%02dz.pgrb2af%03d"
                        % (yr, mo, dy, hr, member, hr, forecast_hour)
                    )
                    for forecast_hour in args["forecast_hours"]
                ]
                ...
                [
                    files.append(
                        "https://noaa-gefs-pds.s3.amazonaws.com/gefs.%d%02d%02d/%02d/pgrb2a/ge%s.t%02dz.pgrb2af%02d"
                        % (yr, mo, dy, hr, member, hr, forecast_hour)
                    )
                    for forecast_hour in args["forecast_hours"]
                ]
                ...
                [
                    files.append(
                        "https://noaa-gefs-pds.s3.amazonaws.com/gefs.%d%02d%02d/%02d/atmos/pgrb2ap5/ge%s.t%02dz.pgrb2a.0p50.f%03d"
                        % (yr, mo, dy, hr, member, hr, forecast_hour)
                    )
                    for forecast_hour in args["forecast_hours"]
                ]
            ...
            if datetime.datetime(yr, mo, dy, hr) < datetime.datetime(2021, 2, 26, 0):
                raise ConnectionAbortedError(
                    "Cannot download GFS data prior to February 26, 2021."
                )
            elif datetime.datetime(yr, mo, dy, hr) < datetime.datetime(2021, 3, 22, 0):
                [
                    files.append(
                        "https://noaa-gfs-bdp-pds.s3.amazonaws.com/gfs.%d%02d%02d/%02d/gfs.t%02dz.pgrb2.0p25.f%03d"
                        % (yr, mo, dy, hr, hr, forecast_hour)
                    )
                    for forecast_hour in args["forecast_hours"]
                ]
                ...
                [
                    files.append(
                        "https://noaa-gfs-bdp-pds.s3.amazonaws.com/gfs.%d%02d%02d/%02d/atmos/gfs.t%02dz.pgrb2.0p25.f%03d"
                        % (yr, mo, dy, hr, hr, forecast_hour)
                    )
                    for forecast_hour in args["forecast_hours"]
                ]
            ...
            if datetime.datetime(yr, mo, dy, hr) < datetime.datetime(2014, 7, 30, 18):
                raise ConnectionAbortedError(
                    "Cannot download HRRR data prior to 18z July 30, 2014."
                )
            [
                files.append(
                    "https://noaa-hrrr-bdp-pds.s3.amazonaws.com/hrrr.%d%02d%02d/conus/hrrr.t%02dz.wrfprsf%02d.grib2"
            ...
            if datetime.datetime(yr, mo, dy, hr) < datetime.datetime(2021, 2, 22, 0):
                raise ConnectionAbortedError(
                    "Cannot download RAP data prior to 0z February 22, 2021."
                )
            [
                files.append(
                    "https://noaa-rap-pds.s3.amazonaws.com/rap.%d%02d%02d/rap.t%02dz.wrfprsf%02d.grib2"
                    % (yr, mo, dy, hr, forecast_hour)
                )
                for forecast_hour in args["forecast_hours"]
            ]
            ...
            [
                files.append(
                    "https://data.ecmwf.int/forecasts/%d%02d%02d/%02dz/%s/0p25/oper/%d%02d%02d%02d0000-%dh-oper-fc.grib2"
                    % (yr, mo, dy, hr, ecmwf_model, yr, mo, dy, hr, forecast_hour)
                )
                for forecast_hour in args["forecast_hours"]
            ]
            ...
            [
                files.append(
                    "https://nomads.ncep.noaa.gov/pub/data/nccf/com/nam/prod/nam.%d%02d%02d/nam.t%02dz.%snest.hiresf%02d.tm00.grib2"
                    % (yr, mo, dy, hr, nest, forecast_hour)
                )
                for forecast_hour in args["forecast_hours"]
            ]
            ...
            for forecast_hour in args["forecast_hours"]:
                ...
                if datetime.datetime(
                    ...
                    files.append(
                        f"https://nomads.ncep.noaa.gov/pub/data/nccf/com/nam/prod/nam.%d%02d%02d/nam.t%02dz.awphys%02d.tm00.grib2"
                        % (yr, mo, dy, hr, forecast_hour)
                    )
                else:
                    files.append(
                        f"https://www.ncei.noaa.gov/data/north-american-mesoscale-model/access/%s/%d%02d/%d%02d%02d/nam_218_%d%02d%02d_%02d00_%03d.grb2"
                        % (folder, yr, mo, yr, mo, dy, yr, mo, dy, hr, forecast_hour)
                    )
        [
            local_filenames.append(
                "%s_%d%02d%02d%02d_f%03d.grib"
                % (args["model"], yr, mo, dy, hr, forecast_hour)
            )
            for forecast_hour in args["forecast_hours"]
        ]
    ...
    for file, local_filename in zip(files, local_filenames):
        ...
        if not os.path.isdir(monthly_directory):
            if (
                requests.head(file).status_code == requests.codes.ok
                or requests.head(file.replace("/atmos", "")).status_code
                == requests.codes.ok
            ):
