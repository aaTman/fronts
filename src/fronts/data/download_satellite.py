...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Output directory for the satellite netCDF files.",
    )
    parser.add_argument(
        "--satellite",
        type=str,
        default="goes16",
        help="Satellite source. Options are 'goes16', 'goes17', 'goes18', 'MERGIR'.",
    )
    parser.add_argument(
        "--domain",
        type=str,
        default="full-disk",
        help="Domain of the satellite data. Options are 'full-disk', 'conus', 'meso'.",
    )
    parser.add_argument(
        "--product",
        type=str,
        default="ABI-L2-MCMIP",
        help="Satellite product to download.",
    )
    parser.add_argument(
        "--init_time",
        type=str,
        help="Initialization time for which to search for satellite data. Format: YYYY-MM-DD-HH.",
    )
    parser.add_argument(
        "--range",
        type=str,
    ...
    if args["satellite"] not in ["goes16", "goes17", "goes18", "MERGIR"]:
        raise ValueError(
            "'%s' is not a valid satellite source. Options are 'goes16', 'goes17', 'goes18', 'MERGIR'."
            % args["satellite"]
        )
    ...
    if args["domain"] == "full-disk":
        ...
        raise ValueError(
            "'%s' is not a valid domain. Options are 'full-disk', 'conus', 'meso'."
            % args["domain"]
        )
    ...
    if args["satellite"] != "MERGIR":
        ...
        args["domain"] = (
            "global"  # force override the domain if downloading merged IR brightness temperature
        )
    ...
    for i, init_time in enumerate(init_times):
        ...
        if args["satellite"] != "MERGIR":
            s3_folder = "s3://noaa-%s/%s%s/%d/%03d/%02d" % (
                args["satellite"],
                args["product"],
                domain_str,
                yr,
            ...
            try:
                files.append(
                    list(sorted(fs.glob(s3_glob_str)))[0].replace(
                        "noaa-%s" % args["satellite"],
                        "https://noaa-%s.s3.amazonaws.com" % args["satellite"],
                    )
                )
            except IndexError:
                print(
                    "NO DATA FOUND (ind %d): satellite=[%s] domain=[%s] product=[%s] init_time=[%s] "
                    % (i, args["satellite"], args["domain"], args["product"], init_time)
                )
                continue
            else:
                print(
                    "Gathering data: satellite=[%s] domain=[%s] product=[%s] init_time=[%s]"
                    % (args["satellite"], args["domain"], args["product"], init_time),
                    end="\r",
                )

        else:
            files.append(
                "https://disc2.gesdisc.eosdis.nasa.gov/data/MERGED_IR/GPM_MERGIR.1/%d/%03d/merg_%d%02d%02d%02d_4km-pixel.nc4"
                % (yr, day_of_year, yr, mo, dy, hr)
            )

        local_filenames.append(
            "%s_%d%02d%02d%02d_%s.nc"
            % (args["satellite"], yr, mo, dy, hr, args["domain"])
        )
    ...
    for file, local_filename in zip(files, local_filenames):
        ...
        monthly_directory = "%s/%s%s" % (
            args["netcdf_outdir"],
            year,
            month,
        )  # Directory for the netCDF files for the given days
