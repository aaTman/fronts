    ...
    * b)`` (a valid convex combination — ``lambda_c >= 0`` and ``sum_c lambda_c == 1``,
    ...
    Args:
        ...
        n_classes: The model's configured class count (``model.ModelConfig.n_classes``). Must
            ...
            ``_build_loss``.
        nbs_pred_buffer_lat_px: Forwarded to every ``_build_loss`` call — see
            ``losses.neighborhood_brier_score``'s ``pred_buffer_lat_px``. Only used by
            "neighborhood_brier_score"; without it, patch-buffer training's wider ``y_pred``
            would hit a shape mismatch inside every per-class loss metric here, one level
            removed from the same crash the compiled loss's own ``pred_buffer_lat_px``/
            ``pred_buffer_lon_px`` exist to prevent.
        nbs_pred_buffer_lon_px: Same as nbs_pred_buffer_lat_px, but for the longitude axis.
    ...
    for key, class_index in class_index_by_key.items():
        ...
        loss_fn = _build_loss(
            ...
            nbs_pred_buffer_lat_px=nbs_pred_buffer_lat_px,
            nbs_pred_buffer_lon_px=nbs_pred_buffer_lon_px,
        )
...
def _compile(
    ...
    Args:
        ...
        pred_buffer_lat_px: Forwarded to ``_build_loss`` (as ``nbs_pred_buffer_lat_px``), both
            ``heidke_skill_score`` calls, and — when ``train_cfg.per_front_type_metrics`` is
            True — to ``metrics.per_front_type_metrics`` and ``_per_front_type_loss_metrics``.
            See ``losses.neighborhood_brier_score``'s ``pred_buffer_lat_px``. 0 (default) is
            correct for whole-domain training.
    ...
    if train_cfg.per_front_type_metrics:
        per_class = metrics.per_front_type_metrics(
            constants.FRONT_TYPE_CLASS_INDEX,
            pred_buffer_lat_px=pred_buffer_lat_px,
            pred_buffer_lon_px=pred_buffer_lon_px,
