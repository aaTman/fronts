...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--project",
        type=str,
        help="WandB project that will be used to store model training data.",
    )
    parser.add_argument(
        "--log_freq",
        type=int,
        default=1,
        help="WandB loss/metric logging frequency in epochs.",
    )
    parser.add_argument(
        "--upload_model", action="store_true", help="Upload model checkpoints to WandB."
    )
    ...
    parser.add_argument(
        "--memory_growth", action="store_true", help="Use memory growth for GPUs"
    )
    parser.add_argument(
        "--num_parallel_calls",
        type=int,
    ...
    parser.add_argument(
        "--disable_tensorfloat32",
        action="store_true",
        help="Disable TensorFloat32 execution.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="How many epochs to complete before validation.",
    )
    ...
    parser.add_argument(
        "--num_training_years",
        type=int,
        help="Number of years for the training dataset.",
    )
    parser.add_argument(
        "--training_years", type=int, nargs="+", help="Years for the training dataset."
    )
    parser.add_argument(
        "--num_validation_years",
        type=int,
        help="Number of years for the validation set.",
    )
    parser.add_argument(
        "--validation_years", type=int, nargs="+", help="Years for the validation set."
    )
    parser.add_argument(
        "--shuffle",
        type=str,
    ...
    incompatible_args = {
        ...
        "unet_3plus": {},
        "attention_unet": dict(
            upsample_size=None, deep_supervision=False, first_encoder_connections=False
        ),
    }
    ...
    metric_args = (
        misc.string_arg_to_dict(args["metric"][1])
        if len(args["metric"]) > 1
        else dict()
    )
    optimizer_args = (
        misc.string_arg_to_dict(args["optimizer"][1])
        if len(args["optimizer"]) > 1
        else dict()
    )
    activity_regularizer_args = (
        misc.string_arg_to_dict(args["activity_regularizer"][1])
        if len(args["activity_regularizer"]) > 1
        else dict()
    )
    bias_constraint_args = (
        misc.string_arg_to_dict(args["bias_constraint"][1])
        if len(args["bias_constraint"]) > 1
        else dict()
    )
    bias_regularizer_args = (
        misc.string_arg_to_dict(args["bias_regularizer"][1])
        if len(args["bias_regularizer"]) > 1
        else dict()
    )
    kernel_constraint_args = (
        misc.string_arg_to_dict(args["kernel_constraint"][1])
        if len(args["kernel_constraint"]) > 1
        else dict()
    )
    kernel_regularizer_args = (
        misc.string_arg_to_dict(args["kernel_regularizer"][1])
    ...
    if len(gpus) > 0:
        ...
        if args["gpu_device"] is not None:
            ...
            gpus = tf.config.get_visible_devices(
                device_type="GPU"
        ...
        if args["memory_growth"]:
            tf.config.experimental.set_memory_growth(
                device=[gpu for gpu in gpus][0], enable=True
            )
    ...
    if args["tf_indirs"][0] != args["tf_indirs"][1]:
        assert (
            train_dataset_properties["front_types"]
            == valid_dataset_properties["front_types"]
        ), (
        ...
        assert (
            train_dataset_properties["variables"]
            == valid_dataset_properties["variables"]
        ), (
        ...
        assert (
            train_dataset_properties["pressure_levels"]
            == valid_dataset_properties["pressure_levels"]
        ), (
        ...
        assert all(
            train_dataset_properties["num_dims"][num]
            == valid_dataset_properties["num_dims"][num]
            for num in range(2)
        ), (
        ...
        assert (
            train_dataset_properties["normalization_parameters"]
            == valid_dataset_properties["normalization_parameters"]
        ), (
    ...
    if not args["retrain"]:
        ...
        if args["num_training_years"] is not None:
            if args["training_years"] is not None:
                raise TypeError(
                    "Cannot explicitly declare the training years if --num_training_years is passed"
                )
            training_years = list(
                sorted(
                    np.random.choice(
                        all_years, args["num_training_years"], replace=False
                    )
                )
            )
        else:
            if args["training_years"] is None:
                raise TypeError(
                    "Must pass one of the following arguments: --training_years, --num_training_years"
                )
        ...
        if args["num_validation_years"] is not None:
            if args["validation_years"] is not None:
                raise TypeError(
                    "Cannot explicitly declare the validation years if --num_validation_years is passed"
                )
            validation_years = list(
                sorted(
                    np.random.choice(
                        [year for year in all_years if year not in training_years],
                        args["num_validation_years"],
                        replace=False,
                    )
                )
            )
            ...
            if args["validation_years"] is None:
                raise TypeError(
                    "Must pass one of the following arguments: --validation_years, --num_validation_years"
                )
            validation_years = list(sorted(args["validation_years"]))

        if len(training_years) + len(validation_years) > len(all_years) - 1:
            raise ValueError(
                "No testing years are available: the total number of training and validation years cannot be greater than 15"
            )

        test_years = [
            year for year in all_years if year not in training_years + validation_years
        ]

        # If no model number was provided, select a number based on the current date and time. This number changes once per minute.
        args["model_number"] = (
            int(datetime.datetime.utcnow().timestamp() % 1e8 / 60)
            if args["model_number"] is None
            else args["model_number"]
        )

        # Convert pool size and upsample size to tuples
        pool_size = tuple(args["pool_size"]) if args["pool_size"] is not None else None
        upsample_size = (
            tuple(args["upsample_size"]) if args["upsample_size"] is not None else None
        )

        if any(
            front_type == front_types
            for front_type in [["MERGED-F_BIN"], ["MERGED-T"], ["F_BIN"]]
        ):
            num_classes = 2
        ...
        model_properties["domains"] = [
            train_dataset_properties["domain"],
            valid_dataset_properties["domain"],
        ]
        ...
        unet_model_args = unet_model.__code__.co_varnames[
            : unet_model.__code__.co_argcount
        ]  # pull argument names from unet function
        ...
        unet_kwargs = {
            ...
            if arg in unet_model_args
        }
        unet_kwargs["squeeze_axes"] = squeeze_axes
        unet_kwargs["activity_regularizer"] = (
            getattr(tf.keras.regularizers, args["activity_regularizer"][0])(
                **activity_regularizer_args
            )
            if args["activity_regularizer"][0] is not None
            else None
        )
        unet_kwargs["bias_constraint"] = (
            getattr(tf.keras.constraints, args["bias_constraint"][0])(
                **bias_constraint_args
            )
            if args["bias_constraint"][0] is not None
            else None
        )
        unet_kwargs["kernel_constraint"] = (
            getattr(tf.keras.constraints, args["kernel_constraint"][0])(
                **kernel_constraint_args
            )
            if args["kernel_constraint"][0] is not None
            else None
        )
        unet_kwargs["bias_regularizer"] = (
            getattr(tf.keras.regularizers, args["bias_regularizer"][0])(
                **bias_regularizer_args
            )
            if args["bias_regularizer"][0] is not None
            else None
        )
        unet_kwargs["kernel_regularizer"] = (
            getattr(tf.keras.regularizers, args["kernel_regularizer"][0])(
                **kernel_regularizer_args
            )
            if args["kernel_regularizer"][0] is not None
            else None
        )
    ...
    model_filepath = "%s/model_%d/model_%d.h5" % (
        args["model_dir"],
        args["model_number"],
        args["model_number"],
    )  # filepath for the actual model (.h5 file)
    history_filepath = "%s/model_%d/model_%d_history.csv" % (
        args["model_dir"],
        args["model_number"],
        args["model_number"],
    )  # path of the CSV file containing loss and metric statistics
    ...
    valid_batch_size = (
        args["batch_size"][0] if len(args["batch_size"]) == 1 else args["batch_size"][1]
    )
    ...
    for year in validation_years:
        validation_files.extend(
            list(sorted(glob(f"{args['tf_indirs'][0]}/{year}-*_tf")))
        )
    ...
    if args["patience"] is None:
        patience = (
            int(images_in_training_dataset / (train_batch_size * train_steps)) + 1
        )
    ...
    with tf.distribute.MirroredStrategy().scope():
        if not args["retrain"]:
            ...
            model.compile(
                loss=loss_function, optimizer=optimizer, metrics=[metric_function]
            )
            ...
            model_properties["metric_child_string"] = (
    ...
    if not args["retrain"] and not args["no_train"]:
        model_properties = {
            key: model_properties[key] for key in sorted(model_properties.keys())
        }  # Sort model properties dictionary alphabetically

        if not os.path.isdir("%s/model_%d" % (args["model_dir"], args["model_number"])):
            os.makedirs(
                "%s/model_%d/maps" % (args["model_dir"], args["model_number"])
            )  # Make folder for model predicton maps
            os.mkdir(
                "%s/model_%d/probabilities" % (args["model_dir"], args["model_number"])
            )  # Make folder for prediction data files
            os.mkdir(
                "%s/model_%d/statistics" % (args["model_dir"], args["model_number"])
            )  # Make folder for statistics data files
        elif not args["override_directory_check"]:
            raise OSError(
                "%s/model_%d already exists. If model %d still needs to be created and trained, run this script "
                "again with the --override_directory_check flag."
                % (args["model_dir"], args["model_number"], args["model_number"])
            )
        elif os.path.isfile(model_filepath):
            raise OSError(
                "model %d already exists at %s. Choose a different model number and try again."
        ...
        with open(
            "%s/model_%d/model_%d_properties.txt"
            % (args["model_dir"], args["model_number"], args["model_number"]),
            "w",
        ) as f:
    ...
    early_stopping = EarlyStopping(
        "val_loss", patience=patience, verbose=1
    )  # EarlyStopping: stops training early if the validation loss does not improve after a specified number of epochs (patience)
    history_logger = CSVLogger(
        history_filepath, separator=",", append=True
    )  # Saves loss and metric data every epoch
    ...
    if not args["no_train"]:
        ...
        if args["project"] is not None:
            ...
            wandb.init(
                project=args["project"], config=wandb_init_config, name=wandb_init_name
            )
            ...
            if args["upload_model"]:
                ...
                )  # upload model checkpoints to wandb
        ...
        print(
            "NOTE: Remove the --no_train argument from the command line to start the training process."
        )
