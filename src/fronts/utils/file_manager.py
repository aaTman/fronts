...
def compress_files(
    ...
    if not isinstance(glob_file_string, str):
        raise TypeError(
            f"glob_file_string must be a string, received {type(glob_file_string)}"
        )
    ...
    if not isinstance(remove_files, bool):
        raise TypeError(
            f"remove_files must be a boolean, received {type(remove_files)}"
        )
    if not isinstance(status_printout, bool):
        raise TypeError(
            f"status_printout must be a boolean, received {type(status_printout)}"
        )
    ####################################################################################################################
    ...
    ### Create the TAR file ###
    with tarfile.open(
        f"{main_dir}/{tar_filename}.tar.{algorithm}", f"w:{algorithm}"
    ) as tarF:
        ### Iterate through all the available files ###
        for file in range(num_files):
            tarF.add(
                files[file], arcname=files[file].replace(main_dir, "")
            )  # Add the file to the TAR file
            tarF_size = (
                os.path.getsize(f"{main_dir}/{tar_filename}.tar.{algorithm}") / 1e6
            )  # Compressed size of the files within the TAR file (megabytes)
            uncompressed_size += (
                os.path.getsize(files[file]) / 1e6
            )  # Uncompressed size of the files within the TAR file (megabytes)

            ### Print out the current status of the compression (if enabled) ###
            if status_printout:
                print(
                    f"({file + 1}/{num_files})      {uncompressed_size:,.2f} MB ---> {tarF_size:,.2f} MB ({100 * (1 - (tarF_size / uncompressed_size)):.1f}% compression ratio)",
                    end="\r",
                )

    # Completion message
    print(
        f"Successfully compressed {len(files)} files: ",
...
def delete_grouped_files(main_dir: str, glob_file_string: str, num_subdir: int):
    ...
    subdir_string = (
        ""  # This string will be modified depending on the provided value of num_subdir
    )
    for _ in range(num_subdir):
        subdir_string += "/*"
    subdir_string += "/"
    glob_file_string = (
        subdir_string + glob_file_string
    )  # String that will be used to match with patterns in filenames

    files_to_delete = list(
        sorted(glob("%s%s" % (main_dir, glob_file_string)))
    )  # Search for files in the given directory that have patterns matching the file string
...
class DataFileLoader:
    ...
    def __init__(
        ...
        if self._file_format in ["grib", "netcdf"]:
            if data_type in ["era5", "gfs"]:
                glob_strs = [
                    f"{file_dir}/{yr}{mo}/{data_type}_{yr}{mo}{dy}{hr}_{domain}{self._file_extension}"
                ...
                glob_strs = [
                    f"{file_dir}/{yr}{mo}/{data_type}_{yr}{mo}{dy}{hr}_*{self._file_extension}"
                    for yr in self._years
                    for mo in self._months
                    for dy in self._days
                    for hr in self._hours
            ...
            glob_strs = [
                f"{file_dir}/{data_type}_{yr}{mo}{self._file_extension}"
                for yr in self._years
                for mo in self._months
            ]
        ...
        glob_strs = [
            glob_str.replace("****", "*").replace("***", "*").replace("**", "*")
    ...
    def add_file_list(self, file_dir, data_type, ignore_domain=False):
        ...
        control_basename_info = [
            file.replace(self._file_extension, "").split("_")[1:]
            for file in control_basename_list
        ]
        current_basename_info = [
            file.replace(self._file_extension, "").split("_")[1:]
            for file in current_basename_list
        ]
        ...
        if len(control_basename_info[0]) == 2:
            if control_data_source in ["era5", "FrontObjects", "goes-merged"]:
                ...
                [
                    file_info.insert(2, control_data_source)
                    for file_info in control_basename_info
                ]
        ...
        if len(current_basename_info[0]) == 2:
            if current_data_source in ["era5", "FrontObjects", "goes-merged"]:
                ...
                [
                    file_info.insert(2, current_data_source)
                    for file_info in current_basename_info
                ]

        # remove the domain from the information arrays (if requested)
        if ignore_domain:
            control_basename_info = [
                file_info[:-1] for file_info in control_basename_info
            ]
            current_basename_info = [
                file_info[:-1] for file_info in current_basename_info
            ]

        # find indices where all file details match
        index_pairs = np.array(
            [
                [data_idx, current_basename_info.index(file_info)]
                for data_idx, file_info in enumerate(control_basename_info)
                if file_info in current_basename_info
            ]
        )

        # filter files and add new list
        self.files = [
            [self.files[list_num][i] for i in index_pairs[:, 0]]
...
def load_model(model_number: int, model_dir: str):
    ...
    if not isinstance(model_number, int):
        raise TypeError(
            f"model_number must be an integer, received {type(model_number)}"
        )
    ...
    model_properties = pd.read_pickle(
        f"{model_dir}/model_{model_number}/model_{model_number}_properties.pkl"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--delete_grouped_files", action="store_true", help="Delete a set of files"
    )
    parser.add_argument(
        "--extract_tarfile", action="store_true", help="Extract a TAR file"
    )
    parser.add_argument(
        "--glob_file_string",
        type=str,
        help="String of the names of the files to compress or delete.",
    )
    parser.add_argument(
        "--main_dir",
        type=str,
        help="Main directory for subdirectory creation or where the files in question are located.",
    )
    parser.add_argument(
        "--num_subdir",
        type=int,
