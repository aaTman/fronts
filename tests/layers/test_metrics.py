...
class TestPerClassContingencyMetricPredBuffer:
    """PerClassContingencyMetric must crop y_pred's patch buffer before accumulating.

    Mirrors TestHeidkeSkillScorePredBufferPx above, but for the stateful contingency
    metric, which has its own crop call (it accumulates counts directly rather than
    calling heidke_skill_score/_crop_pred_buffer).
    """

    def test_lon_buffer_4d_accumulates_same_counts_as_hand_cropped(self):
        """A longitude-only buffered y_pred must accumulate identically to a hand-cropped y_pred."""
        buffer_lon_px = 2
        rng = np.random.default_rng(21)
        labels = rng.integers(0, N_CLASSES, size=(N_BATCH, N_H, N_W))
        y_true = (labels[..., np.newaxis] == np.arange(N_CLASSES)).astype(np.float32)

        rng_pred = np.random.default_rng(22)
        pred_labels = rng_pred.integers(0, N_CLASSES, size=(N_BATCH, N_H, N_W + 2 * buffer_lon_px))
        y_pred_padded = (pred_labels[..., np.newaxis] == np.arange(N_CLASSES)).astype(np.float32)
        y_pred_cropped = y_pred_padded[:, :, buffer_lon_px:-buffer_lon_px, :]

        for statistic in ("hss", "csi", "pod"):
            expected_metric = PerClassContingencyMetric(class_index=CF_CLASS_INDEX, statistic=statistic, threshold=0.5)
            expected_metric.update_state(y_true, y_pred_cropped)
            expected = expected_metric.result().numpy()

            buffered_metric = PerClassContingencyMetric(
                class_index=CF_CLASS_INDEX,
                statistic=statistic,
                threshold=0.5,
                pred_buffer_lat_px=0,
                pred_buffer_lon_px=buffer_lon_px,
            )
            buffered_metric.update_state(y_true, y_pred_padded)
            result = buffered_metric.result().numpy()
            assert result == pytest.approx(expected, abs=1e-5), (
                f"statistic={statistic}: buffered accumulation ({result}) must match hand-cropped ({expected})"
            )

    def test_zero_buffer_on_both_axes_is_true_noop(self, front_pred):
        """pred_buffer_lat_px=0, pred_buffer_lon_px=0 must reproduce the unbuffered accumulation exactly.

        Guards the ``field[:, 0:-0, ...]`` empty-slice trap: naively slicing by ``-buffer`` when
        ``buffer == 0`` yields an empty tensor, not a no-op.
        """
        y_true, y_pred = front_pred
        unbuffered = PerClassContingencyMetric(class_index=CF_CLASS_INDEX, statistic="csi", threshold=0.5)
        unbuffered.update_state(y_true, y_pred)

        explicit_zero = PerClassContingencyMetric(
            class_index=CF_CLASS_INDEX, statistic="csi", threshold=0.5, pred_buffer_lat_px=0, pred_buffer_lon_px=0
        )
        explicit_zero.update_state(y_true, y_pred)

        assert explicit_zero.result().numpy() == pytest.approx(unbuffered.result().numpy(), abs=1e-7)


class TestPerClassContingencyMetric5DPredBuffer:
    """Proves the buffer crop targets the correct two axes on a 5D input.

    For a 5D ``(batch, level, lat, lon, class)`` tensor the spatial axes are (2, 3), NOT
    (1, 2) -- axis 1 is level, not latitude. A crop that reused the 4D-shaped
    ``losses._crop_pred_buffer``-style hardcoded ``[:, buf:-buf, :, :]`` / ``[:, :, buf:-buf, :]``
    slicing would instead crop (level, lat) and leave lon untouched.

    level/lat/lon are deliberately given distinct sizes (5, 3+2*1, 4+2*2) so that a
    wrong-axis crop cannot coincidentally reproduce the right-axis crop's shape: it
    produces (level=3, lat=1, lon=8) against a y_true of (level=5, lat=3, lon=4), which is
    not even broadcastable, let alone numerically correct. So this test cannot pass for
    the wrong axis choice -- it would raise an incompatible-shapes error instead of
    silently returning a plausible-but-wrong number.
    """

    def test_crops_lat_and_lon_axes_not_level(self):
        n_levels, lat_core, lon_core = 5, 3, 4
        buffer_lat_px, buffer_lon_px = 1, 2

        y_true = np.zeros((1, n_levels, lat_core, lon_core, N_CLASSES), dtype=np.float32)
        y_true[..., 0] = 1.0  # background everywhere...
        y_true[:, :, 1, 2, 0] = 0.0
        y_true[:, :, 1, 2, CF_CLASS_INDEX] = 1.0  # ...except one CF pixel at every level.

        y_pred_core = y_true.copy()  # Perfect prediction before buffering.
        y_pred_buffered = np.pad(
            y_pred_core,
            ((0, 0), (0, 0), (buffer_lat_px, buffer_lat_px), (buffer_lon_px, buffer_lon_px), (0, 0)),
            mode="edge",
        )
        # Poison the padded lat/lon border so a crop that misses it (or crops the wrong
        # axes and leaves this border in) pollutes the accumulated counts.
        y_pred_buffered[:, :, :buffer_lat_px, :, :] = 0.0
        y_pred_buffered[:, :, -buffer_lat_px:, :, :] = 0.0
        y_pred_buffered[:, :, :, :buffer_lon_px, :] = 0.0
        y_pred_buffered[:, :, :, -buffer_lon_px:, :] = 0.0
        ...
        metric.update_state(y_true, y_pred_buffered)
        # Perfect prediction after a correct crop: TP = n_levels (one CF pixel per level),
        # FP = FN = 0, so CSI = 1.0 exactly.
        assert metric.result().numpy() == pytest.approx(1.0, abs=1e-5)
...
class TestPerClassContingencyMetricConfig:
    ...
    def test_get_config_round_trips_pred_buffer_px(self):
        """The new pred_buffer_lat_px/pred_buffer_lon_px constructor params must survive serialization.

        Uses a tensor wide enough on both axes to actually exercise the crop (unlike the tiny
        4-pixel-wide tensor in test_get_config_round_trips_through_from_config), so this also
        catches a get_config that reports the right values but a from_config that doesn't
        actually wire them back into cropping behavior.
        """
        buffer_lat_px, buffer_lon_px = 1, 2
        metric = PerClassContingencyMetric(
            class_index=CF_CLASS_INDEX,
        ...
        assert restored.pred_buffer_lat_px == metric.pred_buffer_lat_px == buffer_lat_px
        assert restored.pred_buffer_lon_px == metric.pred_buffer_lon_px == buffer_lon_px

        rng = np.random.default_rng(23)
        core_h, core_w = 4, 4
        labels = rng.integers(0, N_CLASSES, size=(1, core_h + 2 * buffer_lat_px, core_w + 2 * buffer_lon_px))
        y_pred = (labels[..., np.newaxis] == np.arange(N_CLASSES)).astype(np.float32)
        y_true = y_pred[:, buffer_lat_px:-buffer_lat_px, buffer_lon_px:-buffer_lon_px, :]
...
class TestPerFrontTypeMetrics:
    ...
    def test_pred_buffer_forwarded_to_every_metric(self):
        """pred_buffer_lat_px/pred_buffer_lon_px must reach every PerClassContingencyMetric built.

        Otherwise patch-buffer training (y_pred wider than y_true along longitude) crashes the
        first time any of these four-per-front-type metrics runs update_state.
        """
        metrics_list = per_front_type_metrics(
            constants.FRONT_TYPE_CLASS_INDEX, pred_buffer_lat_px=3, pred_buffer_lon_px=5
        )
        assert metrics_list, "expected at least one metric to check"
        for metric in metrics_list:
            assert metric.pred_buffer_lat_px == 3, metric.name
            assert metric.pred_buffer_lon_px == 5, metric.name

    def test_pred_buffer_defaults_to_zero(self):
        """Whole-domain (non-patch) training must keep today's unbuffered behavior by default."""
        metrics_list = per_front_type_metrics(constants.FRONT_TYPE_CLASS_INDEX)
        for metric in metrics_list:
            assert metric.pred_buffer_lat_px == 0
            assert metric.pred_buffer_lon_px == 0
