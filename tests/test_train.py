...
class TestPerFrontTypeLossMetrics:
    ...
    def test_nbs_pred_buffer_forwarded_to_build_loss(self):
        """nbs_pred_buffer_lat_px/lon_px must reach _build_loss for every per-class loss metric.

        Without this, patch-mode training's buffered (wider) y_pred hits the exact
        shape-mismatch crash this task exists to fix -- just one level removed, inside
        every per-class `_per_front_type_loss_metrics` metric rather than the compiled
        loss itself. Proven by comparing against the SAME per-class values computed on
        the un-buffered core prediction with no buffer forwarded: if the buffer argument
        were dropped on the way to `_build_loss`, this call would raise inside
        `neighborhood_brier_score`'s pooling (mismatched y_true/y_pred shapes), not
        merely produce a different number.
        """
        core = 8
        buffer_lon_px = 3
        y_true, y_pred_core = self._fixed_batch(seed=6, height=core, width=core)

        rng = np.random.default_rng(7)
        pad_garbage = tf.nn.softmax(
            rng.standard_normal((2, core, buffer_lon_px, N_CLASSES)).astype(np.float32), axis=-1
        )
        y_pred_buffered = tf.concat([pad_garbage, y_pred_core, pad_garbage], axis=2)

        expected = self._per_class_values("neighborhood_brier_score", None, y_true, y_pred_core, n_classes=N_CLASSES)
        ...
        per_class_metrics = _per_front_type_loss_metrics(
            ...
            nbs_pixel_weight=0.1,
            n_classes=N_CLASSES,
            nbs_pred_buffer_lat_px=0,
            nbs_pred_buffer_lon_px=buffer_lon_px,
        )
        values = []
        for metric in per_class_metrics:
