...
if __name__ == '__main__':
    ...
    parser.add_argument('--year', type=int, required=True, help="Year for the netCDF data to be converted to tensorflow datasets.")
    ...
    parser.add_argument('--num_slices', type=int, default=30, help='Number of slices to perform for each year in the dataset.')
    ...
    file_obj = fm.DataFileLoader(args['variables_indir'], args['data_source'], 'netcdf', years=yr, domains=file_loader_domain)
    ...
    for i_slice, files_in_slice in enumerate(sliced_list, start=1):
        ...
        for input_file, fronts_file in files_in_slice:
            ...
            # open front dataset
            front_dataset = xr.open_dataset(fronts_file, engine='netcdf4').isel(**isel_kwargs)
            ...
            if all_fronts_in_timestep or keep_timestep:
                ...
                if extent_crosses_meridian:
                    ...
                    variables_dataset_1 = xr.open_dataset(input_file, engine='netcdf4')[data_vars].sel(pressure_level=args['pressure_levels'], **sel_kwargs_1).isel(**isel_kwargs).transpose(*transpose_dims, 'pressure_level').astype('float16')
                    variables_dataset_2 = xr.open_dataset(input_file, engine='netcdf4')[data_vars].sel(pressure_level=args['pressure_levels'], **sel_kwargs_2).isel(**isel_kwargs).transpose(*transpose_dims, 'pressure_level').astype('float16')
                    variables_dataset = xr.merge([variables_dataset_1, variables_dataset_2])
    
                else:
                    variables_dataset = xr.open_dataset(input_file, engine='netcdf4')[data_vars].sel(pressure_level=args['pressure_levels'], **sel_kwargs).isel(**isel_kwargs).transpose(*transpose_dims, 'pressure_level').astype('float16')
                ...
                for i_image, image_start_indices in enumerate(image_order):
                    ...
                    if args['verbose']:
                        print(f"{yr}: slice {i_slice}/{num_slices} (kept/discarded):  ({timesteps_kept}/{timesteps_discarded} timesteps, {images_kept}/{images_discarded} images)", end='\r')
                    ...
                    if len(input_tensors_for_slice) == 0:
                        ...
                        input_tensors_for_slice = input_tensors_for_slice.concatenate(input_tensor_for_timestep)
                    ...
                    if len(front_tensors_for_slice) == 0:
                        ...
                        front_tensors_for_slice = front_tensors_for_slice.concatenate(front_tensor_for_timestep)
        ...
        tf_dataset_folder = f"{args['tf_outdir']}/{yr}-{i_slice}_tf"  # output directory for the inputs
        tf_dataset = tf.data.Dataset.zip((input_tensors_for_slice, front_tensors_for_slice))
    ...
    print(f"Tensorflow datasets for {yr} ({num_slices} slices) saved to {args['tf_outdir']}.")