...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_dir", type=str, required=True, help="Parent model directory."
    )
    ...
    init_input_tensor = tf.convert_to_tensor(
        np.random.rand(batch_size, *image_size, N_pressure_levels, N_variables)
    )
    ...
    print(
        "Final benchmark: %.3fs, %.3fs/image"
        % (avg_time_elapsed, avg_time_elapsed / batch_size)
    )
