...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--grib_indir", type=str, required=True, help="Input directory for grib files."
    )
    parser.add_argument(
        "--netcdf_outdir",
        type=str,
        required=True,
        help="Output directory for the netcdf files.",
    )
    parser.add_argument(
        "--model", required=True, type=str, help="NWP model for the grib files."
    )
    parser.add_argument(
        "--init_time",
        type=str,
        required=True,
        help="Initialization time of the model. Format: YYYY-MM-DDTHH.",
    )
    parser.add_argument(
        "--pressure_levels",
        type=str,
        nargs="+",
        default=["surface", "1000", "950", "900", "850"],
        help="Pressure levels to extract from the grib files.",
    )
    parser.add_argument(
        "--gpu",
        action="store_true",
    ...
    parser.add_argument(
        "--ignore_warnings",
        action="store_true",
        help="Disable runtime warnings in variable calculations.",
    )
    ...
    grib_filename_format = f"%s/%d%02d/%s_%d%02d%02d%02d_f*.grib" % (
        args["grib_indir"],
        year,
        month,
        args["model"],
    ...
    open_ds_args = dict(
        engine="cfgrib", errors="ignore", combine="nested", concat_dim="valid_time"
    )
    ...
    if include_pressure_level_data:
        ...
        if (
            args["model"] not in ["nam-12km", "namnest-conus"]
            and "gefs" not in args["model"]
        ):
            pressure_level_data = xr.open_mfdataset(
                grib_files,
                backend_kwargs={"filter_by_keys": {"typeOfLevel": "isobaricInhPa"}},
                **open_ds_args,
            )
            pressure_level_data = pressure_level_data.sel(
            ...
            T = (
                ...
                .sel(isobaricInhPa=pressure_levels)["t"]
                .values
            )  # temperature
            u = (
                xr.open_mfdataset(
                ...
                .sel(isobaricInhPa=pressure_levels)["u"]
                .values
            )  # u-wind
            v = (
                xr.open_mfdataset(
                ...
                .sel(isobaricInhPa=pressure_levels)["v"]
                .values
            )  # v-wind
            RH = (
                xr.open_mfdataset(
                ...
                .sel(isobaricInhPa=pressure_levels)["r"]
                .values
                / 100
            )  # relative humidity
            sp_z = (
                ...
                .sel(isobaricInhPa=pressure_levels)["gh"]
                .values
                / 10
    ...
    if include_surface_data:
        ...
        if args["model"] in ["gfs", "gdas"]:
            sp_data = xr.open_mfdataset(
                grib_files,
                backend_kwargs={
                    "filter_by_keys": {"typeOfLevel": "surface", "stepType": "instant"}
                },
                **open_ds_args,
            )["sp"].values[:, np.newaxis, ...]
            surface_data = xr.open_mfdataset(
                grib_files,
                backend_kwargs={
                    "filter_by_keys": {"typeOfLevel": "sigma", "stepType": "instant"}
                },
                **open_ds_args,
            )
            ...
            surface_data = xr.merge(
                [
                    xr.open_mfdataset(
                        grib_files, filter_by_keys={"paramId": 167}, **open_ds_args
                    )
                    .rename({"t2m": "t"})
                    .drop_vars("heightAboveGround"),
                    xr.open_mfdataset(
                        grib_files, filter_by_keys={"paramId": 165}, **open_ds_args
                    )
                    .rename({"u10": "u"})
                    .drop_vars("heightAboveGround"),
                    xr.open_mfdataset(
                        grib_files, filter_by_keys={"paramId": 166}, **open_ds_args
                    )
                    .rename({"v10": "v"})
                    .drop_vars("heightAboveGround"),
                    xr.open_mfdataset(
                        grib_files, filter_by_keys={"paramId": 260242}, **open_ds_args
                    )
                    .rename({"r2": "r"})
                    .drop_vars("heightAboveGround"),
                ]
        ...
        if include_pressure_level_data:
            ...
            T = np.concatenate(
                [surface_data["t"].values[:, np.newaxis, ...], T], axis=1
            )
            u = np.concatenate(
                [surface_data["u"].values[:, np.newaxis, ...], u], axis=1
            )
            v = np.concatenate(
                [surface_data["v"].values[:, np.newaxis, ...], v], axis=1
            )
            RH = np.concatenate(
                [surface_data["r"].values[:, np.newaxis, ...] / 100, RH], axis=1
            )
    ...
    forecast_hours = [
        int(filename.split("_f")[1][:3]) for filename in grib_files
    ]  # can pull forecast hours straight from the grib filenames
    ...
    full_dataset_variables = dict(
        T=(dataset_dimensions, T),
    ...
    full_dataset = xr.Dataset(
        data_vars=full_dataset_variables, coords=full_dataset_coordinates
    ).astype("float32")

    # final dataset attributes
    full_dataset.attrs = dict(
        grib_to_netcdf_script_version="2024.6.6",
        model=args["model"],
        Nx=np.shape(T)[-1],
        Ny=np.shape(T)[-2],
    )
    ...
    for idx, forecast_hour in enumerate(forecast_hours):
        filepath = f"%s/%s_%d%02d%02d%02d_f%03d.nc" % (
            monthly_dir,
            args["model"],
            year,
            month,
        ...
        ).to_netcdf(path=filepath, mode="w", engine="netcdf4")
