...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Parent input directory for the raw satellite data.",
    )
    parser.add_argument(
        "--init_time",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Output directory for the processed satellite netCDF files.",
    )
    parser.add_argument(
        "--band_nums",
        type=int,
    ...
    parser.add_argument(
        "--multiprocessing",
        action="store_true",
        help="Interpolate the satellite data with one band per CPU thread.",
    )
    parser.add_argument(
    ...
    converted_ds_filepath = "%s/%d%02d/goes-merged_%d%02d%02d%02d_full.nc" % (
        args["netcdf_outdir"],
        year,
        month,
        year,
    ...
    if converted_ds_exists and not args["overwrite"]:
        print(
            "%s already exists. If you want to overwrite the existing dataset, rerun this script with the --overwrite flag attached.\n"
    ...
    try:
        ds_sat1 = xr.open_dataset(
            "%s/%d%02d/%s_%d%02d%02d%02d_full-disk.nc"
            % (args["satellite_indir"], year, month, sat1, year, month, day, hour),
            engine="netcdf4",
        )
        ds_sat2 = xr.open_dataset(
            "%s/%d%02d/%s_%d%02d%02d%02d_full-disk.nc"
            % (args["satellite_indir"], year, month, sat2, year, month, day, hour),
            engine="netcdf4",
        )
    except:
    ...
    for band_num in band_nums:
        ...
        if sat1 == "goes13":
            variable_sat1 = scipy.interpolate.RegularGridInterpolator(
                (lat_sat1, lon_sat1), variable_sat1, method="nearest"
            )((new_lat_sat1, new_lon_sat1))
            variable_sat2 = scipy.interpolate.RegularGridInterpolator(
                (lat_sat2, lon_sat2), variable_sat2, method="nearest"
            )((new_lat_sat2, new_lon_sat2))
        else:
            variable_sat1 = scipy.interpolate.griddata(
                (lat_sat1.ravel(), lon_sat1.ravel()),
                variable_sat1.ravel(),
                (new_lat_sat1, new_lon_sat1),
                method="nearest",
            )
            variable_sat2 = scipy.interpolate.griddata(
                (lat_sat2.ravel(), lon_sat2.ravel()),
                variable_sat2.ravel(),
                (new_lat_sat2, new_lon_sat2),
                method="nearest",
            )
        ...
        overlap_blend = (overlap_sat1 * overlap_mask) + (
            overlap_sat2 * (1 - overlap_mask)
        )

        merged_data = np.vstack(
            [
                variable_sat2[:overlap_west_bound_ind, :],
                overlap_blend,
                variable_sat1[overlap_east_bound_ind + 1 :],
            ]
        )

        ds_merged["band_%d" % band_num] = (
            ("latitude", "longitude"),
            merged_data.astype(np.float32).transpose(),
        )
        ds_merged["band_%d" % band_num].attrs = ds_sat1[band_str].attrs

    os.makedirs(
        "%s/%d%02d" % (args["netcdf_outdir"], year, month), exist_ok=True
    )  # directory check

    print(f"[{dt.datetime.utcnow()}]", "Saving dataset to %s" % converted_ds_filepath)
    ds_merged = ds_merged.expand_dims(
        {"time": np.atleast_1d(init_time).astype("datetime64[ns]")}
    )
    ds_merged = ds_merged.reindex(
        latitude=ds_merged["latitude"].values[::-1]
    )  # reverse latitude values so they are ordered north-south
    ds_merged.to_netcdf(
        converted_ds_filepath, engine="netcdf4", mode="w"
    )  # save dataset
