...
if __name__ == '__main__':
    ...
    for year in years:
        ...
        for month in months:
            ...
            tp_array_temporal = np.zeros(shape=[num_front_types, num_timesteps, 5, 100]).astype('float32')
            fp_array_temporal = np.zeros(shape=[num_front_types, num_timesteps, 5, 100]).astype('float32')
            tn_array_temporal = np.zeros(shape=[num_front_types, num_timesteps, 5, 100]).astype('float32')
            fn_array_temporal = np.zeros(shape=[num_front_types, num_timesteps, 5, 100]).astype('float32')
            tp_array_spatial = np.zeros(shape=[num_front_types, Nlat, Nlon, 5, 100]).astype('float32')
            fp_array_spatial = np.zeros(shape=[num_front_types, Nlat, Nlon, 5, 100]).astype('float32')
            tn_array_spatial = np.zeros(shape=[num_front_types, Nlat, Nlon, 5, 100]).astype('float32')
            fn_array_spatial = np.zeros(shape=[num_front_types, Nlat, Nlon, 5, 100]).astype('float32')
            weights = tf.cast(tf.convert_to_tensor(np.cos(np.deg2rad(lats))[np.newaxis, :, np.newaxis]), tf.float32)  # latitude weights for the statistics

            thresholds = np.linspace(0.01, 1, 100)  # Probability thresholds for calculating performance statistics
            neighborhoods = np.array([50, 100, 150, 200, 250])  # neighborhoods for checking whether a front is present (kilometers)
            ...
            spatial_ds = xr.Dataset(coords={'time': time_array[0], 'latitude': lats, 'longitude': lons, 'neighborhood': neighborhoods, 'threshold': thresholds})
            temporal_ds = xr.Dataset(coords={'time': time_array, 'neighborhood': neighborhoods, 'threshold': thresholds})
            ...
            for front_no, front_type in enumerate(front_types):
                ...
                for i in range(100):
                    ...
                    """
                    tn = tf.cast(tf.where((probs_dss[front_type] < thresholds[i]) & (bool_tn_fn_dss[front_type] == 0), 1, 0), tf.float32) * weights
                    fn = tf.cast(tf.where((probs_dss[front_type] < thresholds[i]) & (bool_tn_fn_dss[front_type] == 1), 1, 0), tf.float32) * weights
                ...
                for neighborhood in range(5):
                    ...
                    print("%d-%02d: %s (%d km)" % (year, month, front_type, (neighborhood + 1) * 50))
                    for i in range(100):
                        """
                        ...
                        """
                        tp = tf.cast(tf.where((probs_dss[front_type] > thresholds[i]) & (bool_tp_fp_dss[front_type] == 1), 1, 0), tf.float32) * weights
                        fp = tf.cast(tf.where((probs_dss[front_type] > thresholds[i]) & (bool_tp_fp_dss[front_type] == 0), 1, 0), tf.float32) * weights

                        tp_array_spatial[front_no, :, :, neighborhood, i] = tf.reduce_sum(tp, axis=0)
                        fp_array_spatial[front_no, :, :, neighborhood, i] = tf.reduce_sum(fp, axis=0)
                        tp_array_temporal[front_no, :, neighborhood, i] = tf.reduce_sum(tp, axis=(1, 2))
                        fp_array_temporal[front_no, :, neighborhood, i] = tf.reduce_sum(fp, axis=(1, 2))

                spatial_ds["tp_spatial_%s" % front_type] = (('latitude', 'longitude', 'neighborhood', 'threshold'), tp_array_spatial[front_no])
                spatial_ds["fp_spatial_%s" % front_type] = (('latitude', 'longitude', 'neighborhood', 'threshold'), fp_array_spatial[front_no])
                spatial_ds["tn_spatial_%s" % front_type] = (('latitude', 'longitude', 'neighborhood', 'threshold'), tn_array_spatial[front_no])
                spatial_ds["fn_spatial_%s" % front_type] = (('latitude', 'longitude', 'neighborhood', 'threshold'), fn_array_spatial[front_no])
                temporal_ds["tp_temporal_%s" % front_type] = (('time', 'neighborhood', 'threshold'), tp_array_temporal[front_no])
                temporal_ds["fp_temporal_%s" % front_type] = (('time', 'neighborhood', 'threshold'), fp_array_temporal[front_no])
                temporal_ds["tn_temporal_%s" % front_type] = (('time', 'neighborhood', 'threshold'), tn_array_temporal[front_no])
                temporal_ds["fn_temporal_%s" % front_type] = (('time', 'neighborhood', 'threshold'), fn_array_temporal[front_no])
            
            spatial_ds.astype('float32').to_netcdf(path=stats_dataset_path.replace('.nc', '_spatial.nc'), mode='w', engine='netcdf4')
            spatial_ds.close()
